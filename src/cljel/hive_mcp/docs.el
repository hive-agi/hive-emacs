;;; hive-mcp-docs.el --- -*- lexical-binding: t; -*-
...
(defun hive-mcp-docs--function-source-file (func)
...
(defun hive-mcp-docs-apropos (pattern &optional type)
  ...
  (let* ((regexp (if (string-match-p "\\\\|\\|\\[\\|\\*\\|\\+" pattern) pattern (regexp-quote pattern)))
        (all-symbols (apropos-internal regexp))
        (filtered (cl-remove-if-not (lambda (sym)
    (pcase type
  ((quote function) (fboundp sym))
  ((quote variable) (boundp sym))
  ((quote command) (commandp sym))
  ((quote face) (facep sym))
  ('_ t))) all-symbols))
    ...
    (list :name (symbol-name sym) :type (hive-mcp-docs--symbol-type sym) :docstring-preview (when doc
    (if (> (length doc) 100) (format "%s..." (substring doc 0 100)) doc))))) limited))))
...
(defun hive-mcp-docs-package-functions (package-or-prefix)
  ...
  (let* ((prefix (if (symbolp package-or-prefix) (symbol-name package-or-prefix) package-or-prefix))
        (prefix-regexp (format "^%s" (regexp-quote prefix)))
        (functions (cl-remove-if-not (lambda (sym)
    (and (fboundp sym) (string-match-p prefix-regexp (symbol-name sym)))) (apropos-internal prefix-regexp (function fboundp))))
...
(defun hive-mcp-docs-find-keybindings (command)
  ...
  (let* ((sym (if (symbolp command) command (intern-soft command))))
    (if (and sym (commandp sym)) (let* ((keys (where-is-internal sym))
        (doc (documentation sym t)))
    (list :command (symbol-name sym) :docstring-preview (when doc
    (if (> (length doc) 200) (format "%s..." (substring doc 0 200)) doc)) :bindings (mapcar (lambda (key)
    (list :key (key-description key))) keys) :binding-count (length keys))) (list :error (format "Command not found: %s" command)))))

(defun hive-mcp-docs-info-lookup (symbol-name)
  "Look up SYMBOL-NAME in Info manuals.\n   Returns relevant Info node content if found."
  (let* ((sym (if (symbolp symbol-name) symbol-name (intern-soft symbol-name))))
    (if (and (featurep 'info-look) sym) (condition-case err
    (let* ((info-lookup-mode 'emacs-lisp-mode)
        (info (info-lookup-symbol sym)))
    (list :symbol (symbol-name sym) :found t :note "Info node opened in Emacs"))
  (error (list :symbol (format "%s" symbol-name) :found nil :note "No Info documentation found"))) (list :symbol (format "%s" symbol-name) :found nil :note "info-look not available or symbol invalid"))))
...
(defun hive-mcp-docs-package-commentary (file-or-feature)
  ...
  (:else nil)))
        (file (when lib-file
    (if (string-suffix-p ".elc" lib-file) (let* ((src (format "%s.el" (file-name-sans-extension lib-file))))
    ...
    (setq commentary-end (or (re-search-forward "^;;; \\(Code\\|History\\|Change ?Log\\):?\\s-*$" nil t) (point-max))))
    (if commentary-start (let* ((raw (buffer-substring-no-properties commentary-start (or commentary-end (point-max))))
        (cleaned (replace-regexp-in-string "^;;+ ?" "" raw)))
    (list :file (file-name-nondirectory file) :path file :commentary (string-trim cleaned))) (list :file (file-name-nondirectory file) :path file :commentary nil :note "No Commentary section found")))) (list :error (format "Could not locate: %s" file-or-feature)))))

(defun hive-mcp-docs-list-packages ()
  "List all loaded features (packages) with their files."
  (let* ((features-info (mapcar (lambda (feat)
    (let* ((file (locate-library (symbol-name feat))))
    (list :name (symbol-name feat) :file (when file
    (file-name-nondirectory file))))) features)))
    (list :total (length features) :features (seq-take features-info 100))))

(defun hive-mcp-docs-describe-at-point ()
...
(defun hive-mcp-docs-apropos-interactive (pattern)
  ...
  (let* ((result (hive-mcp-docs-apropos pattern)))
    ...
    (insert (format "Found %d matches (showing %d)\n\n" (plist-get result :total-matches) (plist-get result :returned)))
    (mapc (lambda (sym)
    (insert (format "%-40s [%s]\n" (plist-get sym :name) (plist-get sym :type)))
    (let* ((doc (plist-get sym :docstring-preview)))
    (when doc
    (insert (format "    %s\n" doc))))) (plist-get result :symbols))
    (goto-char (point-min)))
...
(defun hive-mcp-docs-package-functions-interactive (prefix)
  ...
  (let* ((result (hive-mcp-docs-package-functions prefix)))
    ...
    (insert (format "Found %d functions (showing %d)\n\n" (plist-get result :total-functions) (plist-get result :returned)))
    (mapc (lambda (fn)
    (insert (format "%s%s %s\n" (if (plist-get fn :interactive) "*" " ") (plist-get fn :name) (or (plist-get fn :signature) "")))) (plist-get result :functions))
    (insert "\n* = interactive command\n")
    (goto-char (point-min)))
...
(defun hive-mcp-docs-commentary-interactive (feature)
  ...
  (let* ((result (hive-mcp-docs-package-commentary feature)))
    (if (plist-get result :error) (message "%s" (plist-get result :error)) (progn
  (with-current-buffer (get-buffer-create "*MCP Docs Commentary*")
    ...
    (insert (or (plist-get result :commentary) (plist-get result :note) "No commentary found"))
    (goto-char (point-min)))
  (display-buffer "*MCP Docs Commentary*")))))

(defun hive-mcp-docs--addon-init ()
  "Initialize docs addon."
  (message "hive-mcp-docs: initialized - documentation tools ready"))

(defun hive-mcp-docs--addon-shutdown ()
  "Shutdown docs addon."
  (message "hive-mcp-docs: shutdown"))

(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'docs :version "0.1.0" :description "Emacs documentation lookup tools for MCP" :requires '() :provides '(hive-mcp-docs-describe-function hive-mcp-docs-describe-variable hive-mcp-docs-apropos hive-mcp-docs-package-functions hive-mcp-docs-find-keybindings) :init (function hive-mcp-docs--addon-init) :shutdown (function hive-mcp-docs--addon-shutdown)))

(provide 'hive-mcp-docs)
;;; hive-mcp-docs.el ends here
