...
(ert-deftest hive-mcp-cider-nrepl-test-build-command-cljw-classpath nil "The cljw branch passes the native source roots as one colon-joined -cp.\ncljw's nrepl subcommand takes a single -cp of colon-separated dirs, unlike\ncljrs's repeated --src-path — same configuration, each runtime's own spelling." (let* ((cmd (let* ((hive-mcp-cider-nrepl-cljw-binary "/tmp/cljw")
        (hive-mcp-cider-nrepl-native-source-roots '("src" "test")))
