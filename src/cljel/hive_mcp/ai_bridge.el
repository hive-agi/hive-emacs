...
(defun hive-mcp-ai-bridge-ai-inject-context (&rest args)
  ...
  (let* ((context (hive-mcp-ai-bridge-ai-build-context (when include-semantic
    prompt))))
    (if context (clel-concat context "\n\n---\n\n" prompt) prompt))))
...
(defun hive-mcp-ai-bridge-ai-store-response (&rest args)
  ...
  (when (and hive-mcp-ai-store-notable-responses (hive-mcp-ai-bridge--memory-available-p) (hive-mcp-ai-bridge-ai-response-notable-p response) (hive-mcp-ai-bridge--rate-limit-check (clel-concat "store:" source)))
    (let* ((truncated (if (> (length response) 2000) (clel-concat (substring response 0 1997) "...") response))
...
(defun hive-mcp-ai-bridge-ai-emit-event (event-type data)
  ...
  (when (hive-mcp-ai-bridge--channel-available-p)
    (condition-case err
    (hive-mcp-channel-send (append (list (cons "type" (clel-concat "ai-bridge:" event-type)) (cons "timestamp" (float-time))) data))
