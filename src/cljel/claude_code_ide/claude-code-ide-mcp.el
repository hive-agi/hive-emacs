...
(defun claude-code-ide-mcp--create-lockfile (port project-dir)
  ...
  (let* ((lockfile-dir (claude-code-ide-mcp--lockfile-directory))
        ...
        (lockfile-content (list (cons 'pid (emacs-pid)) (cons 'workspaceFolders workspace-folders) (cons 'ideName "Emacs") (cons 'transport "ws"))))
...
(defun claude-code-ide-mcp--make-error-response (id code message &optional data)
  ...
  (list (cons 'jsonrpc "2.0") (cons 'id id) (cons 'error (append (list (cons 'code code) (cons 'message message)) (when data
    (list (cons 'data data)))))))
...
(defun claude-code-ide-mcp--send-notification (method params)
  ...
  (when-let* ((session (or (when (and claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-session-cache)
    claude-code-ide-mcp--buffer-session-cache) (when-let* ((project-dir (claude-code-ide-mcp--get-buffer-project))
         (found-session (claude-code-ide-mcp--get-session-for-project project-dir)))
    (when found-session
    (setq claude-code-ide-mcp--buffer-session-cache found-session claude-code-ide-mcp--buffer-cache-valid t))
    found-session)))
         (client (claude-code-ide-mcp-session-client session)))
    (let* ((message (list (cons 'jsonrpc "2.0") (cons 'method method) (cons 'params params))))
...
(defun claude-code-ide-mcp--handle-initialize (id _params)
  ...
  (let* ((response (list (cons 'protocolVersion claude-code-ide-mcp-version) (cons 'capabilities (list '(tools . ((listChanged . t))) '(resources . ((subscribe . :json-false) (listChanged . :json-false))) '(prompts . ((listChanged . t))) (cons 'logging (make-hash-table :test 'equal)))) '(serverInfo . ((name . "claude-code-ide-mcp") (version . "0.1.0"))))))
...
(defun claude-code-ide-mcp--handle-tools-list (id _params)
  ...
  (let* ((tools '()))
    ...
    (push (list (cons 'name name) (cons 'description description) (cons 'inputSchema prepared-schema)) tools)))
    (let* ((tools-array (vconcat (nreverse tools)))
        (response (list (cons 'tools tools-array))))
    (claude-code-ide-mcp--make-response id response))))
...
(defun claude-code-ide-mcp--handle-tools-call (id params &optional session)
  ...
  nil) (claude-code-ide-mcp--make-response id (list (cons 'content result))))))
...
(defun claude-code-ide-mcp--handle-message (message &optional session)
  ...
  (error (claude-code-ide-debug "Error sending response: %s" err)))) (claude-code-ide-debug "No client connected, cannot send response"))))
  ((and id (not response)) (claude-code-ide-debug "No response generated for method %s (id %s) - likely deferred" method id))
  (t nil)))))
...
(defun claude-code-ide-mcp--on-open (ws)
  ...
  (let* ((file-path (buffer-file-name))
    ...
    (let* ((file-path (buffer-file-name)))
    (claude-code-ide-mcp--send-notification "workspace/didChangeActiveEditor" (list (cons 'uri (concat "file://" file-path)) (cons 'path file-path) (cons 'name (buffer-name))))))))))) (claude-code-ide-debug "Warning: Could not find session for WebSocket connection"))))
...
(defun claude-code-ide-mcp--track-selection ()
  ...
  (when (buffer-file-name)
    (let* ((session (if (and claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-session-cache) claude-code-ide-mcp--buffer-session-cache (when-let* ((project-dir (claude-code-ide-mcp--get-buffer-project))
         (found-session (claude-code-ide-mcp--get-session-for-project project-dir)))
    (setq claude-code-ide-mcp--buffer-session-cache found-session claude-code-ide-mcp--buffer-cache-valid t)
    found-session))))
    (when session
...
(defun claude-code-ide-mcp--track-active-buffer ()
  ...
  (let* ((current-buffer (current-buffer))
    ...
    (when-let ((session-id (gethash project-dir claude-code-ide--session-ids)))
    (claude-code-ide-mcp-server-update-last-active-buffer session-id current-buffer))
    (claude-code-ide-mcp--send-notification "workspace/didChangeActiveEditor" (list (cons 'uri (concat "file://" file-path)) (cons 'path file-path) (cons 'name (buffer-name current-buffer)))))))))))
...
(defun claude-code-ide-mcp-send-at-mentioned ()
  ...
  (let* ((file-path (or (buffer-file-name) ""))
    ...
    (claude-code-ide-mcp--send-notification "at_mentioned" (list (cons 'filePath file-path) (cons 'lineStart start-line) (cons 'lineEnd end-line)))))
...
(defun claude-code-ide-mcp-complete-deferred (session tool-name result &optional unique-key)
  ...
  (let* ((lookup-key (if unique-key (format "%s-%s" tool-name unique-key) tool-name)))
    ...
    (if client (let* ((response (claude-code-ide-mcp--make-response id (list (cons 'content result))))
