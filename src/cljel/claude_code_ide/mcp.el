;;; claude-code-ide-mcp.el --- -*- lexical-binding: t; -*-
...
(unless (featurep 'websocket)
    (condition-case err
    (require 'websocket)
  (error (claude-code-ide-debug "Failed to load websocket package: %s" (error-message-string err)))))

(defvar claude-code-ide--session-ids nil)

(declare-function claude-code-ide-mcp-handlers--build-tool-list "claude-code-ide-mcp-handlers" (nil))

(declare-function claude-code-ide-mcp-handlers--build-tool-schemas "claude-code-ide-mcp-handlers" (nil))

(declare-function claude-code-ide-mcp-handlers--build-tool-descriptions "claude-code-ide-mcp-handlers" (nil))

(defconst claude-code-ide-mcp-version "2024-11-05" "MCP protocol version.")

(defconst claude-code-ide-mcp-port-range '(10000 . 65535) "Port range for WebSocket server.")

(defconst claude-code-ide-mcp-max-port-attempts 100 "Maximum number of attempts to find a free port.")

(defconst claude-code-ide-mcp-ping-interval 30 "Interval in seconds between ping messages to keep connection alive.")

(defconst claude-code-ide-mcp-selection-delay 0.05 "Delay in seconds before sending selection changes to avoid flooding.")

(defconst claude-code-ide-mcp-initial-notification-delay 0.1 "Delay in seconds before sending initial notifications after connection.")

(defvar claude-code-ide-mcp--sessions (make-hash-table :test 'equal)
  "Hash table mapping project directories to MCP sessions.")

(defvar-local claude-code-ide-mcp--buffer-project-cache nil "Cached project directory for the current buffer.")

(defvar-local claude-code-ide-mcp--buffer-session-cache nil "Cached MCP session for the current buffer.")

(defvar-local claude-code-ide-mcp--buffer-cache-valid nil "Whether the buffer-local cache is valid.")

(define-error 'mcp-error "MCP Error" 'error)

(cl-defstruct claude-code-ide-mcp-session Structure to hold all state for a single MCP session. server client port project-dir deferred ping-timer selection-timer last-selection last-buffer active-diffs original-tab)
...
(defun claude-code-ide-mcp---get-buffer-project ()
  ...
  (if claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-project-cache (let* ((project-dir (when-let ((project (project-current)))
    (expand-file-name (project-root project)))))
    (setq claude-code-ide-mcp--buffer-project-cache project-dir claude-code-ide-mcp--buffer-cache-valid t)
    project-dir)))
...
(defun claude-code-ide-mcp---find-session-by-websocket (ws)
  ...
  (let* ((found-session nil))
    ...
    (setq found-session session))) claude-code-ide-mcp--sessions)
    found-session))
...
(defun claude-code-ide-mcp---active-sessions ()
  ...
  (let* ((sessions '()))
    (maphash (lambda (_project-dir session)
    (push session sessions)) claude-code-ide-mcp--sessions)
    sessions))
...
(defun claude-code-ide-mcp---create-lockfile (port project-dir)
  ...
  (let* ((lockfile-dir (claude-code-ide-mcp--lockfile-directory))
        ...
        (lockfile-content (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/pid) (clojure-core-list '.) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/emacs-pid))))))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/workspaceFolders) (clojure-core-list '.) (clojure-core-list 'clojure.core/workspace-folders)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/ideName) (clojure-core-list '.) (clojure-core-list "Emacs")))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/transport) (clojure-core-list '.) (clojure-core-list "ws"))))))))
    (make-directory lockfile-dir t)
    (condition-case err
    (with-temp-file lockfile-path (insert (json-encode lockfile-content)))
  (error (claude-code-ide-debug "Failed to create lockfile: %s" err)
...
(defun claude-code-ide-mcp---remove-lockfile (port)
  ...
  (when port
    ...
    (if (file-exists-p lockfile-path) (progn
  (delete-file lockfile-path)
  (claude-code-ide-debug "Lockfile deleted: %s" lockfile-path)) (claude-code-ide-debug "Lockfile not found: %s" lockfile-path)))))
...
(defun claude-code-ide-mcp---send-notification (method params)
  ...
  (when-let-star (list session (or (when (and claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-session-cache)
    claude-code-ide-mcp--buffer-session-cache) (when-let-star (list project-dir (claude-code-ide-mcp--get-buffer-project) found-session (claude-code-ide-mcp--get-session-for-project project-dir)) (when found-session
    (setq claude-code-ide-mcp--buffer-session-cache found-session claude-code-ide-mcp--buffer-cache-valid t)) found-session)) client (claude-code-ide-mcp-session-client session)) (let* ((message (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/jsonrpc) (clojure-core-list '.) (clojure-core-list "2.0")))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/method) (clojure-core-list '.) (clojure-core-list 'clojure.core/method)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/params) (clojure-core-list '.) (clojure-core-list 'clojure.core/params))))))))
  ...
  (error (claude-code-ide-debug "Failed to send notification %s: %s" method err))))))
...
(defun claude-code-ide-mcp---prepare-schema-for-json (schema)
  ...
  (cond
  ((eq schema :json-empty) (make-hash-table :test 'equal))
  ((listp schema) (mapcar (lambda (item)
    (if (consp item) (cons (car item) (claude-code-ide-mcp--prepare-schema-for-json (cdr item))) item)) schema))
  (t schema)))
...
(defun claude-code-ide-mcp---handle-tools-list (id _params)
  ...
  (claude-code-ide-debug "Building tools list from %d registered tools" (length claude-code-ide-mcp-tools))
  (let* ((tools '()))
    (dolist (tool-entry claude-code-ide-mcp-tools)
    (let* ((name (car tool-entry))
        (schema (alist-get name claude-code-ide-mcp-tool-schemas nil nil #'string-eq))
        (prepared-schema (claude-code-ide-mcp--prepare-schema-for-json schema))
        (description (alist-get name claude-code-ide-mcp-tool-descriptions nil nil #'string-eq)))
    (push (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/name) (clojure-core-list '.) (clojure-core-list 'clojure.core/name)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/description) (clojure-core-list '.) (clojure-core-list 'clojure.core/description)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/inputSchema) (clojure-core-list '.) (clojure-core-list 'clojure.core/prepared-schema)))))) tools)))
    (let* ((tools-array (vconcat (nreverse tools)))
        (response (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/tools) (clojure-core-list '.) (clojure-core-list 'clojure.core/tools-array))))))))
    (claude-code-ide-mcp--make-response id response))))
...
(defun claude-code-ide-mcp---handle-tools-call (id params &optional session)
  ...
  (let* ((tool-name (alist-get 'name params))
        ...
        (handler (alist-get tool-name claude-code-ide-mcp-tools nil nil #'string-eq)))
  ...
  (let* ((result (if (member tool-name '("getDiagnostics")) (funcall handler arguments session) (funcall handler arguments))))
    (if (alist-get 'deferred result) (progn
  ...
  (let* ((unique-key (alist-get 'unique-key result))
        ...
        (session (or result-session (claude-code-ide-mcp--get-current-session))))
    (if session (let* ((session-deferred (claude-code-ide-mcp-session-deferred session)))
    (puthash storage-key id session-deferred)) (claude-code-ide-debug "Warning: No session found, cannot store deferred response")))
  nil) (claude-code-ide-mcp--make-response id (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/content) (clojure-core-list '.) (clojure-core-list 'clojure.core/result))))))))))
  (mcp-error (claude-code-ide-mcp--make-error-response id -32603 (if (listp (cdr err)) (car (cdr err)) (cdr err))))
  (error (claude-code-ide-mcp--make-error-response id -32603 (format "Tool execution failed: %s" (error-message-string err))))) (progn
  (claude-code-ide-debug "Unknown tool requested: %s" tool-name)
  (claude-code-ide-mcp--make-error-response id -32601 (format "Unknown tool: %s" tool-name))))))
...
(defun claude-code-ide-mcp---handle-message (message &optional session)
  ...
  (when message
    (let* ((method (alist-get 'method message))
        ...
        (response (cond
  ((string= method "initialize") (claude-code-ide-mcp--handle-initialize id params))
  ((string= method "tools/list") (claude-code-ide-mcp--handle-tools-list id params))
  ((string= method "tools/call") (claude-code-ide-mcp--handle-tools-call id params session))
  ((string= method "prompts/list") (claude-code-ide-mcp--handle-prompts-list id params))
  (id (claude-code-ide-mcp--make-error-response id -32601 (format "Method not found: %s" method)))
  (t nil))))
    (cond
  ((response (let* ((client (if session (claude-code-ide-mcp-session-client session) (when-let ((s (or (when (and claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-session-cache)
    claude-code-ide-mcp--buffer-session-cache) (when-let-star (list project-dir (claude-code-ide-mcp--get-buffer-project) found-session (claude-code-ide-mcp--get-session-for-project project-dir)) (when found-session
    (setq claude-code-ide-mcp--buffer-session-cache found-session claude-code-ide-mcp--buffer-cache-valid t)) found-session))))
    (claude-code-ide-mcp-session-client s)))))
    (if client (let* ((response-text (json-encode response)))
    (condition-case err
    (websocket-send-text client response-text)
  (error (claude-code-ide-debug "Error sending response: %s" err)))) (claude-code-ide-debug "No client connected, cannot send response")))) ((and id (not response)) (claude-code-ide-debug "No response generated for method %s (id %s) - likely deferred" method id)))))))
...
(defun claude-code-ide-mcp---find-free-port ()
  ...
  (let* ((min-port (car claude-code-ide-mcp-port-range))
    ...
    (let* ((ws-server (websocket-server port :host "127.0.0.1" :on-open #'-on-open :on-message #'-on-message :on-error #'-on-error :on-close #'-on-close :on-ping #'-on-ping :protocol '("mcp"))))
    (when (and ws-server claude-code-ide-debug)
    (set-process-filter ws-server (lambda (proc string)
    (if (string-match-p "^[[:print:][:space:]]+$" string) (claude-code-ide-debug "Server received text data: %S" string) (claude-code-ide-debug "Server received binary frame (%d bytes)" (length string)))
    (websocket-server-filter proc string))))
    ws-server)
  (error (claude-code-ide-debug "Failed to bind to port %d: %s" port err)
      nil))))
    (if server (progn
  (setq found-port (cons server port))
  (claude-code-ide-debug "WebSocket server started on port %d" port)) (cl-incf attempts))))
    (or found-port (error "Could not find free port in range %d-%d" min-port max-port))))
...
(defun claude-code-ide-mcp---on-open (ws)
  ...
  (let* ((session nil)
    ...
    (setq session s))) claude-code-ide-mcp--sessions))
    (if session (progn
  (setf (claude-code-ide-mcp-session-client session) ws)
  (claude-code-ide-debug "Claude Code connected to MCP server for %s" (file-name-nondirectory (directory-file-name (claude-code-ide-mcp-session-project-dir session))))
  (let* ((file-path (buffer-file-name))
        (project-dir (claude-code-ide-mcp-session-project-dir session)))
    (when (and file-path project-dir (string-prefix-p (expand-file-name project-dir) (expand-file-name file-path)))
    ...
    (run-at-time claude-code-ide-mcp-initial-notification-delay nil (lambda (_unused)
    (when-let ((s (gethash project-dir claude-code-ide-mcp--sessions)))
    (let* ((file-path (buffer-file-name)))
    (claude-code-ide-mcp--send-notification "workspace/didChangeActiveEditor" (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/uri) (clojure-core-list '.) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/concat) (clojure-core-list "file://") (clojure-core-list 'clojure.core/file-path))))))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/path) (clojure-core-list '.) (clojure-core-list 'clojure.core/file-path)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/name) (clojure-core-list '.) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/buffer-name))))))))))))))))) (claude-code-ide-debug "Warning: Could not find session for WebSocket connection"))))
...
(defun claude-code-ide-mcp---on-message (ws frame)
  ...
  (let* ((session (claude-code-ide-mcp--find-session-by-websocket ws)))
    (if session (let* ((text (websocket-frame-text frame))
        (message (condition-case err
    (json-read-from-string text)
  (error nil))))
    (when message
    (claude-code-ide-mcp--handle-message message session))) (claude-code-ide-debug "Warning: Could not find session for WebSocket message"))))
  (error (claude-code-ide-debug "Error processing WebSocket frame: %s" err)
      nil)))
...
(defun claude-code-ide-mcp---on-close (ws)
  ...
  (let* ((session (claude-code-ide-mcp--find-session-by-websocket ws)))
    ...
    (claude-code-ide-debug "Claude Code disconnected from MCP server for %s" (file-name-nondirectory (directory-file-name (claude-code-ide-mcp-session-project-dir session)))))))
...
(defun claude-code-ide-mcp---start-ping-timer (session)
  ...
  (claude-code-ide-mcp--stop-ping-timer session)
  (let* ((timer (run-with-timer claude-code-ide-mcp-ping-interval claude-code-ide-mcp-ping-interval (lambda (_unused)
...
(defun claude-code-ide-mcp---send-ping (session)
  ...
  (when-let ((client (claude-code-ide-mcp-session-client session)))
    (condition-case err
    (websocket-send client (make-websocket-frame :opcode 'ping :payload ""))
  (error (claude-code-ide-debug "Failed to send ping: %s" err)))))

(defun claude-code-ide-mcp---invalidate-buffer-cache ()
  "Invalidate the buffer-local cache for project and session."
  (setq claude-code-ide-mcp--buffer-project-cache nil claude-code-ide-mcp--buffer-session-cache nil claude-code-ide-mcp--buffer-cache-valid nil))
...
(defun claude-code-ide-mcp---track-selection ()
  ...
  (when (buffer-file-name)
    (let* ((session (if (and claude-code-ide-mcp--buffer-cache-valid claude-code-ide-mcp--buffer-session-cache) claude-code-ide-mcp--buffer-session-cache (when-let-star (list project-dir (claude-code-ide-mcp--get-buffer-project) found-session (claude-code-ide-mcp--get-session-for-project project-dir)) (setq claude-code-ide-mcp--buffer-session-cache found-session claude-code-ide-mcp--buffer-cache-valid t) found-session))))
    (when session
    ...
    (setf (claude-code-ide-mcp-session-selection-timer session) (run-with-timer claude-code-ide-mcp-selection-delay nil (lambda (_unused)
...
(defun claude-code-ide-mcp---send-selection-for-project (project-dir)
  ...
  (when-let ((session (claude-code-ide-mcp--get-session-for-project project-dir)))
    ...
    (let* ((file-path (buffer-file-name)))
    (when (and (claude-code-ide-mcp-session-client session) file-path)
    (let* ((file-in-project (string-prefix-p (expand-file-name project-dir) (expand-file-name file-path))))
    (if file-in-project (let* ((cursor-pos (point))
        (current-state (if (use-region-p) (list cursor-pos (region-beginning) (region-end)) (list cursor-pos cursor-pos cursor-pos)))
    ...
    (claude-code-ide-mcp--send-notification "selection_changed" selection)))) (setf (claude-code-ide-mcp-session-last-selection session) nil))))
    (unless file-path
...
(defun claude-code-ide-mcp---track-active-buffer ()
  ...
  (let* ((current-buffer (current-buffer))
    ...
    (let* ((project-dir (claude-code-ide-mcp-session-project-dir session)))
    (when (and (not (eq current-buffer (claude-code-ide-mcp-session-last-buffer session))) (string-prefix-p (expand-file-name project-dir) (expand-file-name file-path)))
    (setf (claude-code-ide-mcp-session-last-buffer session) current-buffer)
    (when-let ((session-id (gethash project-dir claude-code-ide--session-ids)))
    (claude-code-ide-mcp-server-update-last-active-buffer session-id current-buffer))
...
(defun claude-code-ide-mcp-start (&optional project-directory)
  ...
  (let* ((project-dir (expand-file-name (or project-directory default-directory)))
    ...
    (if existing-session (progn
  (claude-code-ide-debug "Reusing existing session for %s" project-dir)
  (claude-code-ide-mcp-session-port existing-session)) (let* ((session (make-claude-code-ide-mcp-session :project-dir project-dir :deferred (make-hash-table :test 'equal) :active-diffs (make-hash-table :test 'equal) :original-tab (when (fboundp 'tab-bar--current-tab)
    (tab-bar--current-tab))))
        (server-and-port (claude-code-ide-mcp--find-free-port))
        (server (car server-and-port))
        (port (cdr server-and-port)))
    (setf (claude-code-ide-mcp-session-port session) port (claude-code-ide-mcp-session-server session) server)
    ...
    (claude-code-ide-debug "MCP server started on port %d for %s" port (file-name-nondirectory (directory-file-name project-dir)))
    port))))
...
(defun claude-code-ide-mcp-stop-session (project-dir)
  ...
  (when-let ((session (gethash project-dir claude-code-ide-mcp--sessions)))
    ...
    (when (and claude-code-ide-mcp--buffer-project-cache (string= claude-code-ide-mcp--buffer-project-cache project-dir))
    (claude-code-ide-mcp--invalidate-buffer-cache))))
    (when (equal 0 (hash-table-count claude-code-ide-mcp--sessions))
    (remove-hook 'post-command-hook #'-track-selection)
    (remove-hook 'post-command-hook #'-track-active-buffer))
    (claude-code-ide-debug "MCP server stopped for %s" (file-name-nondirectory (directory-file-name project-dir)))))
...
(defun claude-code-ide-mcp-stop ()
  ...
  (let* ((project-dir (claude-code-ide-mcp--get-buffer-project)))
    (if project-dir (claude-code-ide-mcp-stop-session project-dir) (let* ((sessions (hash-table-keys claude-code-ide-mcp--sessions)))
    (if sessions (dolist (dir sessions)
    (claude-code-ide-mcp-stop-session dir)) (claude-code-ide-debug "No MCP servers running"))))))

(defun claude-code-ide-mcp-send-at-mentioned ()
  "Send at-mentioned notification."
  (let* ((file-path (or (buffer-file-name) ""))
        (start-line (if (use-region-p) (1- (line-number-at-pos (region-beginning))) (1- (line-number-at-pos (point)))))
        (end-line (if (use-region-p) (1- (line-number-at-pos (region-end))) (1- (line-number-at-pos (point))))))
    (claude-code-ide-mcp--send-notification "at_mentioned" (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/filePath) (clojure-core-list '.) (clojure-core-list 'clojure.core/file-path)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/lineStart) (clojure-core-list '.) (clojure-core-list 'clojure.core/start-line)))) (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/lineEnd) (clojure-core-list '.) (clojure-core-list 'clojure.core/end-line)))))))))
...
(defun claude-code-ide-mcp-complete-deferred (session tool-name result &optional unique-key)
  ...
  (let* ((lookup-key (if unique-key (format "%s-%s" tool-name unique-key) tool-name)))
    ...
    (if (not session) (claude-code-ide-debug "No session provided for completing deferred response %s" lookup-key) (let* ((session-deferred (claude-code-ide-mcp-session-deferred session))
        (id (gethash lookup-key session-deferred)))
    (if id (let* ((client (claude-code-ide-mcp-session-client session)))
    (remhash lookup-key session-deferred)
    (if client (let* ((response (claude-code-ide-mcp--make-response id (clel-seq (clel-concat (clojure-core-list (clel-seq (clel-concat (clojure-core-list 'clojure.core/content) (clojure-core-list '.) (clojure-core-list 'clojure.core/result))))))))
        (json-response (json-encode response)))
    (claude-code-ide-debug "Sending deferred response: %s" json-response)
    (websocket-send-text client json-response)) (claude-code-ide-debug "No client connected for session, cannot send deferred response"))) (claude-code-ide-debug "No deferred response found for %s" lookup-key))))))

(defun claude-code-ide-mcp---cleanup ()
  "Cleanup all MCP sessions on Emacs exit."
  (maphash (lambda (project-dir _session)
    (claude-code-ide-mcp-stop-session project-dir)) claude-code-ide-mcp--sessions))
