...
(defun hive-mcp-gptel-directive-with-context ()
  ...
  (when (hive-mcp-gptel--ensure-bridge)
    ...
    (cons "Memory-Aware" (concat "You are a helpful assistant with access to project knowledge.\n\n" (or context "") "\n\nUse this context to provide informed, project-specific responses.")))))
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'gptel :version "0.1.0" :description "gptel integration with memory and swarm" :requires '(hive-mcp-ai-bridge) :provides '(hive-mcp-gptel-mode) :init #'hive-mcp-gptel--addon-init :shutdown #'hive-mcp-gptel--addon-shutdown))
