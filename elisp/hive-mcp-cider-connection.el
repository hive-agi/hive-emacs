...
(defcustom hive-mcp-cider-connection-spawn-port-retries 5
  "How many times a spawn that lost its port to another process retries on another port."
  :group 'hive-mcp-cider
  :type 'integer)
...
(defun hive-mcp-cider-connection-port-in-use-p (tail)
  "True when a dead spawn's last output says its port was already bound.\nThat is a race, not a broken launch: another JVM took the port between the\nfree-port probe and this process's bind, so the spawn can simply try another\nport. TAIL is the process buffer's last output, or nil. Pure."
  (and tail (string-match-p "Address already in use" tail) t))
...
(defun hive-mcp-cider-connection--apply-settled-props (name props)
  ...
  (when (hive-mcp-cider-sessions-exists-p name)
    (clel-apply #'hive-mcp-cider-sessions-update-props name props)
...
(defun hive-mcp-cider-connection-watch-spawn-process (name process &optional on-port-in-use)
  ...
  (when (processp process)
    ...
    (let* ((tail (hive-mcp-cider-connection--process-output-tail (format "*nREPL-%s*" name) 4))
        (reason (hive-mcp-cider-connection-spawn-death-reason (string-trim (or event "exited")) tail)))
    (hive-mcp-cider-connection--cancel-session-timer name)
    (if (and on-port-in-use (hive-mcp-cider-connection-port-in-use-p tail)) (progn
  (message "hive-mcp-cider: Session '%s' lost its port to another process; retrying on another port" name)
  (funcall on-port-in-use)) (progn
