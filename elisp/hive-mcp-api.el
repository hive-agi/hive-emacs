...
(defun hive-mcp-api-get-context ()
  ...
  (hive-mcp-with-fallback (let* ((ctx (hive-mcp-context-full)))
    (plist-put ctx :memory (hive-mcp-with-fallback (hive-mcp-memory-get-project-context) nil))
    ctx) (list :error "context-unavailable" :buffer (buffer-name) :point (point))))
...
(defun hive-mcp-api-memory-query (type &optional tags limit scope-filter)
  ...
  (hive-mcp-with-fallback (let* ((scope-arg (hive-mcp-transform-scope-arg scope-filter))
        (results (hive-mcp-memory-query (intern type) tags nil (or limit 20) nil scope-arg)))
    (hive-mcp-transform-entries-to-vector results)) (list )))
...
(defun hive-mcp-api-memory-add (type content &optional tags duration)
  ...
  (hive-mcp-with-fallback (let* ((entry (hive-mcp-memory-add (intern type) content tags nil (when duration
    (intern duration)))))
    (hive-mcp-transform-plist-to-alist entry)) (hive-mcp-transform-error-result "memory-add-failed" 'type nil 'id nil)))
...
(defun hive-mcp-api-memory-query-metadata (type &optional tags limit scope-filter)
  ...
  (let* ((scope-arg (hive-mcp-transform-scope-arg scope-filter))
        (results (hive-mcp-memory-query (intern type) tags nil (or limit 20) nil scope-arg)))
    (clel-apply #'vector (mapcar #'hive-mcp-transform-entry-to-metadata results))))
...
(defun hive-mcp-api-run-workflow (name &optional args)
  ...
  (hive-mcp-with-fallback (if (fboundp 'hive-mcp-workflow-run) (hive-mcp-workflow-run name args) '((error . "workflows-not-available"))) (hive-mcp-transform-error-result "workflow-execution-failed" 'workflow name)))
...
(defun hive-mcp-api-workflow-wrap (&optional accomplishments decisions conventions in-progress next-actions completed-tasks)
  ...
  (hive-mcp-with-fallback (if (fboundp 'hive-mcp-workflow-wrap) (let* ((result (hive-mcp-workflow-wrap (list :accomplishments accomplishments :decisions decisions :conventions conventions :in-progress in-progress :next-actions next-actions :completed-tasks completed-tasks))))
    (hive-mcp-transform-plist-to-alist result)) '((error . "wrap-workflow-not-available"))) '((error . "wrap-workflow-failed"))))
...
(defun hive-mcp-api-workflow-catchup ()
  ...
  (hive-mcp-with-fallback (if (fboundp 'hive-mcp-workflow-catchup) (let* ((result (hive-mcp-workflow-catchup)))
    (hive-mcp-transform-plist-to-alist result)) '((error . "catchup-workflow-not-available"))) '((error . "catchup-workflow-failed"))))
...
(defun hive-mcp-api-wrap-gather (&optional directory)
  ...
  (hive-mcp-with-fallback (if (fboundp 'hive-mcp-workflow-wrap--gather-session-data) (let* ((result (hive-mcp-workflow-wrap--gather-session-data directory)))
    (hive-mcp-transform-plist-to-alist result)) '((error . "wrap-gather-not-available"))) '((error . "wrap-gather-failed"))))
...
(defun hive-mcp-api-open-file (path &optional line)
  ...
  (hive-mcp-with-fallback (progn
  (find-file path)
  (when line
    (goto-char (point-min))
    (forward-line (1- line)))
  t) (hive-mcp-transform-error-result "file-open-failed" 'path path)))
...
(defun hive-mcp-api-kanban-create (title &optional priority context directory)
  ...
  (hive-mcp-with-fallback (let* ((project-id (when directory
    (hive-mcp-memory-project-id directory)))
        (entry (hive-mcp-kanban-task-create title priority context project-id)))
    (hive-mcp-transform-plist-to-alist entry)) (hive-mcp-transform-error-result "kanban-create-failed" 'title title)))
...
(defun hive-mcp-api-kanban-list (&optional status directory)
  ...
  (hive-mcp-with-fallback (let* ((project-id (when directory
    ...
    (hive-mcp-transform-entries-to-vector entries)) (list )))
...
(defun hive-mcp-api-kanban-move (task-id new-status &optional directory)
  ...
  (hive-mcp-with-fallback (let* ((project-id (when directory
    ...
    (if (eq result t) '((deleted . t) (status . "done")) (hive-mcp-transform-plist-to-alist result))) (hive-mcp-transform-error-result "kanban-move-failed" 'task_id task-id)))
...
(defun hive-mcp-api-kanban-delete (task-id &optional directory)
  ...
  (hive-mcp-with-fallback (let* ((project-id (when directory
    (hive-mcp-memory-project-id directory))))
    (if (hive-mcp-kanban-task-delete task-id project-id) '((deleted . t)) '((deleted . :false)))) (hive-mcp-transform-error-result "kanban-delete-failed" 'task_id task-id)))
...
(defun hive-mcp-api-kanban-stats (&optional directory)
  ...
  (hive-mcp-with-fallback (let* ((project-id (when directory
    ...
    (hive-mcp-transform-stats-to-alist stats '(:todo :doing :review))) (hive-mcp-transform-error-result "kanban-stats-failed")))
...
(defun hive-mcp-api-call-tool (tool-name params)
  ...
  (let* ((idx (string-match "_" tool-name))
        ...
        (command (when idx
    (substring tool-name (1+ idx))))
        (merged-params (if command (append (list :command command) params) params))
        (result (hive-mcp-api-call tool merged-params)))
    (when result
    (cond
  ((plist-get result :isError) (list :error (or (plist-get result :text) "unknown error")))
  ((and (equal (plist-get result :type) "text") (plist-get result :text)) (condition-case nil
    (json-parse-string (plist-get result :text) :object-type 'plist :null-object nil)
  (error result)))
  (t result)))))
