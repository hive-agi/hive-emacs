...
(defun olympus-ui--render-grid (state)
  ...
  (let* ((layout (plist-get state :layout))
    ...
    (clel-apply 'concat (cl-loop for row from 0 below rows collect (let* ((row-lines nil))
    ...
    (let* ((cells (clel-reverse row-lines))
        (merged-lines nil))
    ...
    (push (string-join (clel-reverse line-parts) "  ") merged-lines)))
    (concat (string-join (clel-reverse merged-lines) "\n") "\n\n")))))))
