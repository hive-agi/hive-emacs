...
(defun hive-mcp-hooks--log (format-string &rest args)
  ...
  (when hive-mcp-hooks-log-events
    (clel-apply #'message (concat "[hive-mcp-hooks] " format-string) args)))
...
(defun hive-mcp-hooks--notify (format-string &rest args)
  ...
  (clel-apply #'hive-mcp-hooks-notify-function format-string args))
...
(defun hive-mcp-hooks-trigger-wrap (action &optional session-id)
  ...
  (hive-mcp-hooks-emit-hook-event "hook/wrap" (list (cons "action" action) (cons "session-id" session-id) (cons "timestamp" (truncate (float-time))))))

(defun hive-mcp-hooks-trigger-notify (message &optional level title)
  "Trigger a notification hook with MESSAGE, LEVEL, and TITLE."
  (hive-mcp-hooks-emit-hook-event "hook/notify" (list (cons "message" message) (cons "level" (or level "info")) (cons "title" title))))
...
(with-eval-after-load 'hive-mcp-addons
  ...
  (hive-mcp-addon-register 'hooks :version "0.1.0" :description "Hook event processor for channel-based triggers" :requires '(hive-mcp-channel) :provides '(hive-mcp-hooks-init hive-mcp-hooks-process) :init #'hive-mcp-hooks--addon-init :shutdown #'hive-mcp-hooks--addon-shutdown))
