...
(defun hive-mcp-kanban-board--render ()
  ...
  (let* ((inhibit-read-only t)
    ...
    (insert (propertize "                                              ║\n" 'face 'hive-mcp-kanban-border))
    (insert (propertize "╠═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╣\n" 'face 'hive-mcp-kanban-border))
    (let* ((total (clel-apply #'+ (mapcar (lambda (col)
    ...
    (insert (propertize "╣\n" 'face 'hive-mcp-kanban-border))
    (let* ((max-tasks (clel-apply #'max 1 (mapcar (lambda (col)
    (length (cdr (assoc (car col) tasks-by-status)))) columns))))
...
(defun hive-mcp-kanban-board--load-tasks-cider (file)
  ...
  (let* ((code (format "(do (require '[hive-mcp.org-clj.parser :as p])\n                           (require '[hive-mcp.org-clj.query :as q])\n                           (let [doc (p/parse-document (slurp \"%s\"))]\n                             {:todo (q/find-todo doc)\n                              :in-progress (q/find-in-progress doc)\n                              :in-review (q/find-by-status doc \"IN-REVIEW\")\n                              :done (q/find-done doc)}))" file))
    ...
    (when value
    (let* ((data (read value)))
    (backquote (("TODO" - (mapcar #'hive-mcp-kanban-board--convert-task (plist-get data :todo))) ("IN-PROGRESS" - (mapcar #'hive-mcp-kanban-board--convert-task (plist-get data :in-progress))) ("IN-REVIEW" - (mapcar #'hive-mcp-kanban-board--convert-task (plist-get data :in-review))) ("DONE" - (mapcar #'hive-mcp-kanban-board--convert-task (plist-get data :done)))))))))
...
(defun hive-mcp-kanban-board--move-task (direction)
  ...
  (when-let* ((id (get-text-property (point) 'task-id))
    ...
    (let* ((cols '("TODO" "IN-PROGRESS" "IN-REVIEW" "DONE"))
        ...
        (new-col (and (>= new-idx 0) (< new-idx 4) (clel-nth cols new-idx))))
    (when new-col
...
(defun hive-mcp-kanban-board-move-task ()
  ...
  (interactive)
  (when-let* ((id (get-text-property (point) 'task-id)))
    (let* ((new-status (completing-read "Move to: " '("TODO" "IN-PROGRESS" "IN-REVIEW" "DONE") nil t))
