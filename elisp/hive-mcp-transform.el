...
(defun hive-mcp-transform--convert-value (val)
  ...
  ((and (listp val) val (listp (car-safe val)) (keywordp (car-safe (car-safe val)))) (clel-apply #'vector (mapcar #'hive-mcp-transform-plist-to-alist val)))
  ((and (listp val) val) (clel-apply #'vector (mapcar (lambda (v)
...
(defun hive-mcp-transform-entries-to-vector (entries)
  ...
  (clel-apply #'vector (mapcar #'hive-mcp-transform-plist-to-alist entries)))
...
(defun hive-mcp-transform-entry-to-metadata (entry)
  ...
  (let* ((content (plist-get entry :content))
    ...
    (list (cons 'id (plist-get entry :id)) (cons 'type (plist-get entry :type)) (cons 'preview (hive-mcp-transform-content-preview content)) (cons 'tags (if tags (clel-apply #'vector tags) (list ))) (cons 'created (plist-get entry :created)))))
