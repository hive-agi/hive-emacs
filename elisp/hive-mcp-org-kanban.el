...
(let* ((this-dir (file-name-directory (or load-file-name buffer-file-name))))
    (add-to-list 'load-path (expand-file-name "kanban" this-dir)))
(require 'cl-lib)
(require 'org)
(require 'json)
...
(defun hive-mcp-org-kanban-update-task (task-id &rest props)
  ...
  (let* ((result (clel-apply #'hive-mcp-kanban-protocol--update-task hive-mcp-org-kanban-backend task-id props)))
    (when hive-mcp-org-kanban-auto-sync
    (clel-apply #'hive-mcp-org-kanban--sync-update-to-other-backend task-id props))
    result))
...
(defun hive-mcp-org-kanban--sync-update-to-other-backend (task-id &rest props)
  ...
  (let* ((other-backend (if (eq hive-mcp-org-kanban-backend 'vibe) 'standalone 'vibe)))
    (clel-apply #'hive-mcp-kanban-protocol--update-task other-backend task-id props)))
