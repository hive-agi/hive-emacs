...
(defun hive-mcp-cci--handle-hivemind-completion (task-id status result files)
  ...
  (when-let* ((task (gethash task-id hive-mcp-cci--tasks)))
    ...
    (hive-mcp-cci--emit-event "task-completed" (list (cons "task-id" task-id) (cons "status" status) (cons "ling-id" (plist-get task :ling-id))))
...
(defun hive-mcp-cci--emit-event (event-type data)
  ...
  (when (and (fboundp 'hive-mcp-channel-connected-p) (hive-mcp-channel-connected-p))
    (ignore-errors (hive-mcp-channel-send (list (cons "type" (format "cci-%s" event-type)) (cons "data" data) (cons "timestamp" (format-time-string "%FT%T%z")))))))
...
(cl-defun hive-mcp-cci-spawn (name &key presets cwd hivemind-agent)
  ...
  (error (when-let* ((ling (gethash ling-id hive-mcp-cci--lings)))
...
(defun hive-mcp-cci-status ()
  ...
  ((quote error) (cl-incf error-count)))
    ...
    (let* ((status (list :backend "claude-code-ide" :completion-mechanism "hivemind" :auto-sync hive-mcp-cci-auto-sync :lings (list :total total :idle idle :working working :error error-count) :tasks (list :total (hash-table-count hive-mcp-cci--tasks) :pending (hash-table-count hive-mcp-cci--pending-completions)) :lings-detail (nreverse lings-detail))))
...
(defun hive-mcp-cci-api-spawn (name presets &optional cwd)
  ...
  (error (list :error "spawn-failed" :reason (error-message-string err)))))
...
(defun hive-mcp-cci-api-dispatch (ling-id prompt &optional timeout-ms)
  ...
  (error (list :error "dispatch-failed" :reason (error-message-string err)))))
...
(defun hive-mcp-cci-api-collect (task-id)
  ...
  (if-let* ((task (gethash task-id hive-mcp-cci--tasks)))
    task
  (list :error "task-not-found" :task-id task-id)))
