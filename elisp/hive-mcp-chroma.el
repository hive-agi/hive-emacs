...
(defun hive-mcp-chroma--project-root ()
  ...
  ((and (boundp 'hive-mcp-root) hive-mcp-root) hive-mcp-root)
  (t (let* ((load-path-dir (file-name-directory (or load-file-name buffer-file-name default-directory))))
...
(defun hive-mcp-chroma-start ()
  ...
  (let* ((compose-file (hive-mcp-chroma--docker-compose-file)))
    ...
    (setq hive-mcp-chroma--process (clel-apply #'start-process "hive-mcp-chroma" "*hive-mcp-chroma*" (car cmd) args))
...
(defun hive-mcp-chroma-stop ()
  ...
  (let* ((compose-file (hive-mcp-chroma--docker-compose-file))
    ...
    (clel-apply #'call-process (car cmd) nil nil nil args)
    (setq hive-mcp-chroma--status 'stopped)
...
(defun hive-mcp-chroma--semantic-search (query limit type)
  ...
  (when (fboundp 'hive-mcp--send-request)
    (hive-mcp--send-request "tools/call" (list :name "cider_eval_silent" :arguments (list :code (format "(do\n        (require '[hive-mcp.chroma :as chroma])\n        (chroma/search-similar %S :limit %d %s))" query limit (if type (format ":type %S" type) "")))) (lambda (response)
...
(defun hive-mcp-chroma--local-search (query limit type)
  ...
  (when (fboundp 'hive-mcp--send-request)
    (hive-mcp--send-request "tools/call" (list :name "mcp_memory_query" :arguments (list :type (or type "note") :limit (or limit 20))) (lambda (response)
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'chroma :version "0.1.0" :description "Chroma vector database for semantic memory search" :requires '(hive-mcp-api) :provides '(hive-mcp-chroma-mode hive-mcp-chroma-transient) :init #'hive-mcp-chroma--addon-init :shutdown #'hive-mcp-chroma--addon-shutdown))
