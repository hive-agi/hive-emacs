...
(defun hive-mcp-swarm--sync-state-from-hivemind (event-data)
  ...
  (let* ((agent-id (plist-get event-data :agent-id))
    ...
    (let* ((candidate-buf (get-buffer (format "%s%s*" hive-mcp-swarm-buffer-prefix agent-id)))
        (live-buf (when (and candidate-buf (buffer-live-p candidate-buf))
    candidate-buf))
        (new-slave (list :id agent-id :name agent-id :status 'working :depth 1 :spawn-mode (if live-buf "claude" "headless") :buffer live-buf :cwd nil)))
    (puthash agent-id new-slave hive-mcp-swarm--slaves)
    (setq slave new-slave)
    (message "[swarm-sync] Auto-registered agent '%s' (event: %s, buffer: %s)" agent-id event-type (if live-buf "found" "none"))))
    (when slave
...
(defalias 'hive-mcp-swarm-list-presets 'hive-mcp-swarm-presets-list-presets)
...
(defalias 'hive-mcp-swarm--slave-ready-p 'hive-mcp-swarm-tasks--slave-ready-p)
...
(defun hive-mcp-swarm-api-spawn (ling-name presets &optional cwd terminal kanban-task-id context-file)
  ...
  (let* ((valid-cwd (when (stringp cwd)
    ...
    (hive-mcp-with-fallback (hive-mcp-swarm-spawn ling-name :presets presets :cwd valid-cwd :terminal (when (and terminal (stringp terminal))
    (intern terminal)) :kanban-task-id valid-kanban-id :injected-context injected-context) (list :error "spawn-failed" :name ling-name :reason "unknown"))))
...
(defun hive-mcp-swarm-api-dispatch (slave-id prompt &optional timeout-ms)
  ...
  (hive-mcp-with-fallback (hive-mcp-swarm-dispatch slave-id prompt :timeout timeout-ms) (list :status "error" :error "dispatch-failed" :slave-id slave-id :reason "unknown")))
...
(defun hive-mcp-swarm-api-collect (task-id &optional timeout-ms)
  ...
  (list :task-id task-id :status "polling" :elapsed-ms elapsed-ms :timeout-ms task-timeout :message "Task still running, poll again"))))) (list :task-id task-id :status "error" :result nil :error "collection-failed")))
...
(defun hive-mcp-swarm-api-kill (slave-id)
  ...
  (hive-mcp-with-fallback (if (hive-mcp-swarm-kill slave-id) (list :killed slave-id :success t) (list :error "kill-blocked" :slave-id slave-id :reason "buffer-safety-validation-failed" :message "Buffer did not pass safety checks (may have been coordinator or invalid target)")) (list :error "kill-failed" :slave-id slave-id)))
...
(defun hive-mcp-swarm-api-kill-all ()
  ...
  (hive-mcp-with-fallback (let* ((count (hash-table-count hive-mcp-swarm--slaves)))
    ...
    (list :killed-count count)) (list :error "kill-all-failed" :killed-count 0)))
...
(defun hive-mcp-swarm-api-respond-prompt (slave-id response)
  ...
  (hive-mcp-with-fallback (if (hive-mcp-swarm-prompts-respond-to slave-id response) (list :success t :slave-id slave-id :response response) (list :success nil :error "no-pending-prompt" :slave-id slave-id)) (list :error "respond-prompt-failed" :slave-id slave-id)))
