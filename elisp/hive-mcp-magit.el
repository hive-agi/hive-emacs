...
(defun hive-mcp-magit--get-recent-commits (&optional count)
  "Return last COUNT commits as list of plists.\nA field the log line does not carry reads as \"\", never a signal."
  (let* ((n (or count hive-mcp-magit-log-count))
    ...
    (list :hash (clel-nth parts 0 "") :short-hash (clel-nth parts 1 "") :author (clel-nth parts 2 "") :email (clel-nth parts 3 "") :subject (clel-nth parts 4 "") :date (clel-nth parts 5 "")))) lines)))

(defun hive-mcp-magit--get-ahead-behind ()
  "Return commits ahead/behind upstream as plist.\nA count the rev-list line does not carry reads as 0, never a signal."
  (let* ((result (hive-mcp-magit--shell-command "git rev-list --left-right --count @{upstream}...HEAD 2>/dev/null")))
    ...
    (list :behind (string-to-number (clel-nth parts 0 "0")) :ahead (string-to-number (clel-nth parts 1 "0")))))))
...
(defun hive-mcp-magit--push (&optional set-upstream remote)
  "Push to remote.  SET-UPSTREAM to set tracking if needed.\nREMOTE names the remote to push the current branch to; blank or absent, git's\nown default remote is used.  With REMOTE the refspec is always explicit, so\nthe push and the message it prints name the remote the caller asked for."
  (hive-mcp-magit--ensure-repo)
  (let* ((branch (hive-mcp-magit--get-current-branch))
        (upstream (hive-mcp-magit--get-upstream-branch))
        (cmd (if (and remote (not (string-empty-p remote))) (format "git push %s%s %s 2>&1" (if set-upstream "-u " "") (shell-quote-argument remote) (shell-quote-argument branch)) (if (or upstream (not set-upstream)) "git push 2>&1" (format "git push -u origin %s 2>&1" (shell-quote-argument branch)))))
...
(defun hive-mcp-magit-api-push (&optional options directory)
  "Push to remote.\nOPTIONS may contain :set-upstream to set tracking and :remote to name the\nremote to push to.\nDIRECTORY overrides `default-directory' if provided."
  (let* ((default-directory (or directory default-directory)))
    (hive-mcp-magit--ensure-repo)
    (hive-mcp-magit--push (plist-get options :set-upstream) (plist-get options :remote))))
...
(defun hive-mcp-magit-status ()
  ...
  (let* ((status (hive-mcp-magit-api-status))
    ...
    (insert "\n")
    (when (or (> (plist-get status :ahead) 0) (> (plist-get status :behind) 0))
    (insert (format "[ahead %d, behind %d]\n" (plist-get status :ahead) (plist-get status :behind))))
    (insert "\n")
...
(define-minor-mode hive-mcp-magit-mode
  ...
  :group 'hive-mcp-magit
  (if hive-mcp-magit-mode (message "hive-mcp-magit enabled") (message "hive-mcp-magit disabled")))
...
(defun hive-mcp-magit--addon-init ()
  ...
  (message "hive-mcp-magit: initialized%s" (if (featurep 'magit) " (with Magit)" " (shell fallback)")))
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'magit :version "0.1.0" :description "Magit Git integration with shell fallback" :requires '(hive-mcp-api) :provides '(hive-mcp-magit-mode hive-mcp-magit-transient) :init #'hive-mcp-magit--addon-init :shutdown #'hive-mcp-magit--addon-shutdown))
