...
(defun hive-mcp-projectile-api-list-projects ()
  ...
  (if (hive-mcp-projectile--ensure-projectile) (let* ((projects (or (and (boundp 'projectile-known-projects) projectile-known-projects) nil)))
    (clel-apply #'vector (mapcar (lambda (root)
...
(defun hive-mcp-projectile-api-project-files (&optional pattern)
  ...
  (if (hive-mcp-projectile--in-project-p) (clel-apply #'vector (hive-mcp-projectile--list-files pattern)) (list )))
...
(defun hive-mcp-projectile-api-find-file (filename)
  ...
  (if (hive-mcp-projectile--in-project-p) (let* ((matches (hive-mcp-projectile--find-file-matches filename))
    ...
    (clel-apply #'vector (mapcar (lambda (f)
    (list :relative f :absolute (expand-file-name f root))) matches))) (list )))

(defun hive-mcp-projectile-api-recent-files ()
  "Get recently visited files in current project."
  (if (hive-mcp-projectile--in-project-p) (clel-apply #'vector (or (hive-mcp-projectile--recent-files) '())) (list )))

(defun hive-mcp-projectile-api-search (pattern)
  "Search current project for PATTERN."
  (if (hive-mcp-projectile--in-project-p) (clel-apply #'vector (hive-mcp-projectile--search pattern)) (list )))
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'projectile :version "0.1.0" :description "Projectile project management integration" :requires '(projectile hive-mcp-api) :provides '(hive-mcp-projectile-mode hive-mcp-projectile-transient) :init #'hive-mcp-projectile--addon-init :shutdown #'hive-mcp-projectile--addon-shutdown))
