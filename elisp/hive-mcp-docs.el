...
(defun hive-mcp-docs-apropos (pattern &optional type)
  ...
  (_ t))) all-symbols))
    ...
    (list :name (symbol-name sym) :type (hive-mcp-docs--symbol-type sym) :docstring-preview (when-let* ((doc (hive-mcp-docs--get-docstring sym)))
    (if (> (length doc) 100) (concat (substring doc 0 100) "...") doc)))) limited))))
...
(defun hive-mcp-docs-find-keybindings (command)
  ...
  (let* ((sym (if (symbolp command) command (intern-soft command))))
    ...
    (list :command (symbol-name sym) :docstring-preview (when-let* ((doc (documentation sym t)))
    (if (> (length doc) 200) (concat (substring doc 0 200) "...") doc)) :bindings (mapcar (lambda (key)
...
(defun hive-mcp-docs-list-packages ()
  ...
  (let* ((features-info (mapcar (lambda (feat)
    (list :name (symbol-name feat) :file (when-let* ((file (locate-library (symbol-name feat))))
