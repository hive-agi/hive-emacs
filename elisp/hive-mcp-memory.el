...
(defun hive-mcp-memory--build-clj-call (op args)
  ...
  (pcase op
  ((quote add) (let* ((type (clel-nth 0 args))
        (content (clel-nth 1 args))
        (tags (clel-nth 2 args))
        (project-id (clel-nth 3 args))
        (duration (clel-nth 4 args)))
  ...
  ((quote get) (let* ((id (clel-nth 0 args))
        (project-id (clel-nth 1 args)))
    (format "(hive-mcp.tools.memory/handle-memory {:command \"get\" :id %S :directory %s})" id (if project-id (format "\"%s\"" project-id) "nil"))))
  ((quote query) (let* ((type (clel-nth 0 args))
        (tags (clel-nth 1 args))
        (project-id (clel-nth 2 args))
        (limit (clel-nth 3 args))
        (duration (clel-nth 4 args))
        (scope-filter (clel-nth 5 args)))
  ...
  ((quote search) (let* ((query (clel-nth 0 args))
        (project-id (clel-nth 1 args))
        (limit (clel-nth 2 args)))
  ...
  ((quote metadata) (let* ((type (clel-nth 0 args))
        (tags (clel-nth 1 args))
        (project-id (clel-nth 2 args))
        (limit (clel-nth 3 args)))
  ...
  ((quote update) (let* ((id (clel-nth 0 args))
        (updates (clel-nth 1 args))
        (_project-id (clel-nth 2 args)))
  ...
  ((quote migrate) (let* ((old-project-id (clel-nth 0 args))
        (new-project-id (clel-nth 1 args)))
