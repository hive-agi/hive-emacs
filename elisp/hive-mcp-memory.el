...
(defun hive-mcp-memory--duration-next (duration)
  ...
  (let* ((pos (seq-position hive-mcp-memory-durations duration)))
    ...
    (clel-nth (1+ pos) hive-mcp-memory-durations))))
...
(defun hive-mcp-memory--build-clj-call (op args)
  ...
  (pcase op
  ((quote add) (let* ((type (clel-nth args 0))
        (content (clel-nth args 1))
        (tags (clel-nth args 2))
        (project-id (clel-nth args 3))
        (duration (clel-nth args 4)))
  ...
  ((quote get) (let* ((id (clel-nth args 0))
        (project-id (clel-nth args 1)))
    (format "(hive-mcp.tools.memory/handle-memory {:command \"get\" :id %S :directory %s})" id (if project-id (format "\"%s\"" project-id) "nil"))))
  ((quote query) (let* ((type (clel-nth args 0))
        (tags (clel-nth args 1))
        (project-id (clel-nth args 2))
        (limit (clel-nth args 3))
        (duration (clel-nth args 4))
        (scope-filter (clel-nth args 5)))
  ...
  ((quote search) (let* ((query (clel-nth args 0))
        (project-id (clel-nth args 1))
        (limit (clel-nth args 2)))
  ...
  ((quote metadata) (let* ((type (clel-nth args 0))
        (tags (clel-nth args 1))
        (project-id (clel-nth args 2))
        (limit (clel-nth args 3)))
  ...
  ((quote update) (let* ((id (clel-nth args 0))
        (updates (clel-nth args 1))
        (_project-id (clel-nth args 2)))
  ...
  ((quote migrate) (let* ((old-project-id (clel-nth args 0))
        (new-project-id (clel-nth args 1)))
...
(defun hive-mcp-memory--delegate (&rest clel--args)
  ...
  (if hive-mcp-memory--storage-delegate (clel-apply #'hive-mcp-memory--storage-delegate operation args) (error "hive-mcp-memory: Storage delegate not configured. Use MCP tools (mcp_memory_add, mcp_memory_query) directly, or set hive-mcp-memory--storage-delegate"))))
