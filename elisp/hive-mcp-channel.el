...
(defun hive-mcp-channel--bencode-fallback (obj)
  ...
  ((listp obj) (if (and (consp (car obj)) (not (listp (cdar obj)))) (concat "d" (mapconcat (lambda (pair)
    (concat (hive-mcp-channel--bencode-fallback (if (symbolp (car pair)) (symbol-name (car pair)) (car pair))) (hive-mcp-channel--bencode-fallback (cdr pair)))) obj "") "e") (concat "l" (mapconcat #'hive-mcp-channel--bencode-fallback obj "") "e")))
  ((vectorp obj) (concat "l" (mapconcat #'hive-mcp-channel--bencode-fallback (append obj nil) "") "e"))
...
(defun hive-mcp-channel--bdecode-fallback (str)
  ...
  ((equal c 105) (progn
  (cl-incf pos)
  (let* ((end (cl-position 101 str :start pos)))
    (prog1 (string-to-number (substring str pos end)) (setq pos (1+ end))))))
  ((and (>= c 48) (<= c 57)) (let* ((colon (cl-position 58 str :start pos)))
  ...
  (cl-incf pos)
  (let* ((items nil))
    (while (not (equal (aref str pos) 101))
...
(defun hive-mcp-channel--sentinel (_proc event)
  ...
  (cond
  (hive-mcp-channel--degraded-mode nil)
  ((and (> hive-mcp-channel-max-reconnects 0) (>= hive-mcp-channel--reconnect-count hive-mcp-channel-max-reconnects)) (progn
  ...
  (hive-mcp-channel-reconnect-interval (progn
...
(defun hive-mcp-channel-dispatch-local (event-type data)
  ...
  (let* ((type-key (if (keywordp event-type) event-type (intern (concat ":" event-type))))
...
(defun hive-mcp-channel-emit-swarm-event (event-type data)
  ...
  (hive-mcp-channel-send (append (list (cons "type" (if (keywordp event-type) (substring (symbol-name event-type) 1) event-type)) (cons "timestamp" (float-time))) data)))
...
(defun hive-mcp-channel-status ()
  ...
  (interactive)
  (let* ((status (list (cons 'connected (hive-mcp-channel-connected-p)) (cons 'degraded hive-mcp-channel--degraded-mode) (cons 'reconnect-count hive-mcp-channel--reconnect-count) (cons 'queued-messages (length hive-mcp-channel--message-queue)) (cons 'type hive-mcp-channel-type))))
