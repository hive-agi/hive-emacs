...
(defun hive-mcp-ellama--build-context-prompt (prompt)
  ...
  (if (and hive-mcp-ellama-inject-context (hive-mcp-ellama--ensure-bridge)) (let* ((context (hive-mcp-ai-build-context (when hive-mcp-ellama-include-semantic
    prompt))))
    (if context (concat (hive-mcp-ai-format-context-for 'ellama context) hive-mcp-ellama-context-prefix prompt) prompt)) prompt))
...
(defun hive-mcp-ellama--chat-advice (orig-fun prompt &rest args)
  ...
  (let* ((augmented (if hive-mcp-ellama-inject-context (hive-mcp-ellama--build-context-prompt prompt) prompt)))
    (clel-apply orig-fun augmented args)))
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'ellama :version "0.1.0" :description "Ollama integration via ellama for two-tier swarm" :requires '(hive-mcp-ai-bridge) :provides '(hive-mcp-ellama-mode) :init #'hive-mcp-ellama--addon-init :shutdown #'hive-mcp-ellama--addon-shutdown))
