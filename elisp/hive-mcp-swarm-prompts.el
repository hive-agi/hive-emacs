...
(defun hive-mcp-swarm-prompts--send-approval (buffer term-type)
  ...
  (when (buffer-live-p buffer)
    ...
    (goto-char (point-max))
    (pcase term-type
  ((quote vterm) (vterm-send-string "y") (run-at-time 0.05 nil (lambda ()
    (condition-case err
    (when (buffer-live-p buffer)
    (with-current-buffer buffer
    (vterm-send-return)))
...
(defun hive-mcp-swarm-prompts-update-buffer ()
  ...
  (when-let* ((buf (get-buffer hive-mcp-swarm-prompts-buffer-name)))
    ...
    (if hive-mcp-swarm-prompts--pending (dolist (prompt (clel-reverse hive-mcp-swarm-prompts--pending))
...
(defun hive-mcp-swarm-prompts-approve ()
  ...
  (interactive)
  (if-let* ((prompt (clel-pop hive-mcp-swarm-prompts--pending)))
    (progn
...
(defun hive-mcp-swarm-prompts-start-watcher (slaves-hash get-terminal-fn)
  ...
  (setq hive-mcp-swarm-prompts--timer (run-with-timer 1 2 (lambda ()
    (condition-case err
