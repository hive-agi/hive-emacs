...
(defmacro claude-code-ide-tests--with-mocked-cli (cli-path &rest body)
  ...
  (clel-seq (clel-concat (list 'clojure.core/let) (list (apply clojure-core-vector (clel-seq (clel-concat (list 'clojure.core/claude-code-ide-cli-path) (list 'clojure.core/cli-path) (list 'clojure.core/claude-code-ide--cli-available) (list 'nil))))) (list (clel-seq (clel-concat (list 'clojure.core/deref) (list 'clojure.core/body)))))))
...
(ert-deftest claude-code-ide-test-terminal-send-functions (nil) "Test terminal send wrapper functions." (let* ((vterm-string-sent nil)
        (vterm-escape-sent nil)
        (vterm-return-sent nil)
        (eat-string-sent nil))
    (cl-letf (((symbol-function 'vterm-send-string) (lambda (str)
    (setq vterm-string-sent str))) ((symbol-function 'vterm-send-escape) (lambda ()
    (setq vterm-escape-sent t))) ((symbol-function 'vterm-send-return) (lambda ()
    (setq vterm-return-sent t))) ((symbol-function 'eat-term-send-string) (lambda (term str)
    (setq eat-string-sent str)))) (let* ((claude-code-ide-terminal-backend 'vterm))
    ...
    (should vterm-return-sent)) (with-temp-buffer
    ...
    (claude-code-ide--terminal-send-escape)
    (should (equal eat-string-sent ""))
    (setq eat-string-sent nil)
    (claude-code-ide--terminal-send-return)
    (should (equal eat-string-sent "\r")))))))

(ert-deftest claude-code-ide-test-check-status (nil) "Test status check command." (let* ((claude-code-ide-cli-path "echo")
        (claude-code-ide--cli-available nil))
    (claude-code-ide-check-status)
    (should claude-code-ide--cli-available)))

(ert-deftest claude-code-ide-test-stop-no-session (nil) "Test stop command when no session is running." (claude-code-ide-tests--clear-processes) (unwind-protect
    (claude-code-ide-tests--with-temp-directory (lambda (_unused)
    (claude-code-ide-stop)))
  (claude-code-ide-tests--clear-processes)))

(ert-deftest claude-code-ide-test-switch-to-buffer-no-session (nil) "Test switch-to-buffer command when no session exists." (claude-code-ide-tests--clear-processes) (unwind-protect
    (should-error (claude-code-ide-switch-to-buffer) :type 'user-error)
  (claude-code-ide-tests--clear-processes)))

(ert-deftest claude-code-ide-test-list-sessions-empty (nil) "Test listing sessions when none exist." (claude-code-ide-tests--clear-processes) (unwind-protect
    (claude-code-ide-list-sessions)
  (claude-code-ide-tests--clear-processes)))

(ert-deftest claude-code-ide-test-debug-mode-flag (nil) "Test debug mode CLI flag." (let* ((claude-code-ide-cli-debug t))
...
(ert-deftest claude-code-ide-test-error-handling (nil) "Test error handling in various scenarios." (let* ((claude-code-ide-cli-path nil)
        (claude-code-ide--cli-available nil))
    (should-error (claude-code-ide) :type 'user-error)) (let* ((claude-code-ide-cli-path "")
        (claude-code-ide--cli-available nil))
    (should-error (claude-code-ide) :type 'user-error)))

(ert-deftest claude-code-ide-test-window-placement-options (nil) "Test different window placement configurations." (dolist (side '(left right top bottom))
    (let* ((claude-code-ide-window-side side))
    (should (eq claude-code-ide-window-side side)))))

(defun claude-code-ide-tests-run-tests ()
  "Run all claude-code-ide test cases."
  (interactive)
  (ert-run-tests-batch-and-exit "^claude-code-ide-test-"))

(defun claude-code-ide-tests-run-all-tests ()
...
(defmacro claude-code-ide-mcp-tests--with-temp-file (file-var content &rest body)
  ...
  (progn
  (defvar claude-code-ide-tests-(indent 2)))
  (clel-seq (clel-concat (list 'clojure.core/let) (list (apply clojure-core-vector (clel-seq (clel-concat (list file-var) (list (clel-seq (clel-concat (list 'clojure.core/make-temp-file) (list "claude-mcp-test-")))))))) (list (clel-seq (clel-concat (list 'clojure.core/unwind-protect) (list (clel-seq (clel-concat (list 'clojure.core/progn) (list (clel-seq (clel-concat (list 'clojure.core/with-temp-file) (list file-var) (list (clel-seq (clel-concat (list 'clojure.core/insert) (list content))))))) (list (clel-seq (clel-concat (list 'clojure.core/deref) (list 'clojure.core/body))))))) (list (clel-seq (clel-concat (list 'clojure.core/delete-file) (list file-var))))))))))
...
(defmacro claude-code-ide-mcp-tests--with-temp-buffer (content &rest body)
  ...
  (progn
  (defvar claude-code-ide-tests-(indent 1)))
  (clel-seq (clel-concat (list 'clojure.core/with-temp-buffer) (list (clel-seq (clel-concat (list 'clojure.core/insert) (list content)))) (list (clel-seq (clel-concat (list 'clojure.core/goto-char) (list (clel-seq (clel-concat (list 'clojure.core/point-min))))))) (list (clel-seq (clel-concat (list 'clojure.core/deref) (list 'clojure.core/body)))))))

(ert-deftest claude-code-ide-test-mcp-open-file (nil) "Test the openFile tool implementation." (claude-code-ide-mcp-tests--with-temp-file test-file "Line 1\nLine 2\nLine 3\nLine 4" (let* ((result (claude-code-ide-mcp-handle-open-file (list (cons 'path test-file)))))
    (should (listp result))
    ...
    (kill-buffer))) (should-error (claude-code-ide-mcp-handle-open-file '()) :type 'mcp-error))

(ert-deftest claude-code-ide-test-mcp-get-current-selection (nil) "Test the getCurrentSelection tool implementation." (claude-code-ide-mcp-tests--with-temp-buffer "Line 1\nLine 2\nLine 3" (goto-char (point-min)) (set-mark (point)) (forward-line 2) (let* ((transient-mark-mode t))
    (activate-mark)
    ...
    (should (equal (alist-get 'line start) 1))
    (should (equal (alist-get 'line end) 3))))))) (claude-code-ide-mcp-tests--with-temp-buffer "Test" (let* ((result (claude-code-ide-mcp-handle-get-current-selection nil)))
    (should (equal (alist-get 'text result) ""))
    (let* ((selection (alist-get 'selection result)))
    (should selection)
    (should (alist-get 'isEmpty selection))))))

(ert-deftest claude-code-ide-test-mcp-tool-registry (nil) "Test that all tools are properly registered." (let* ((base-tools '("openFile" "getCurrentSelection" "getOpenEditors" "getWorkspaceFolders" "getDiagnostics" "saveDocument" "close_tab" "checkDocumentDirty"))
    ...
    (should (or (functionp handler) (and (symbolp handler) (fboundp handler))))
    (should schema)))))

(ert-deftest claude-code-ide-test-mcp-server-lifecycle (nil) "Test MCP server start and stop." (require 'claude-code-ide-mcp) (unwind-protect
    (progn
...
(ert-deftest claude-code-ide-test-diagnostics-severity-mapping (nil) "Test diagnostic severity conversion." (require 'claude-code-ide-diagnostics) (should (equal (claude-code-ide-diagnostics--severity-to-vscode 'error) 1)) (should (equal (claude-code-ide-diagnostics--severity-to-vscode 'warning) 2)) (should (equal (claude-code-ide-diagnostics--severity-to-vscode 'info) 3)) (should (equal (claude-code-ide-diagnostics--severity-to-vscode 'hint) 4)) (should (equal (claude-code-ide-diagnostics--severity-to-vscode 'unknown) 3)))

(ert-deftest claude-code-ide-test-diagnostics-severity-to-string (nil) "Test severity to string conversion." (require 'claude-code-ide-diagnostics) (should (equal (claude-code-ide-diagnostics--severity-to-string 'error) "Error")) (should (equal (claude-code-ide-diagnostics--severity-to-string 'warning) "Warning")) (should (equal (claude-code-ide-diagnostics--severity-to-string 'info) "Information")) (should (equal (claude-code-ide-diagnostics--severity-to-string 'hint) "Hint")) (should (equal (claude-code-ide-diagnostics--severity-to-string 'unknown) "Information")))

(ert-deftest claude-code-ide-test-diagnostics-handler (nil) "Test getDiagnostics handler." (require 'claude-code-ide-diagnostics) (let* ((result (claude-code-ide-diagnostics-handler nil)))
    (should (listp result))
    (should (equal (alist-get 'type (car result)) "text"))
    (should (equal (alist-get 'text (car result)) "[]"))))

(ert-deftest claude-code-ide-test-check-document-dirty (nil) "Test checkDocumentDirty handler." (require 'claude-code-ide-mcp-handlers) (with-temp-buffer
    (setq buffer-file-name "/tmp/test-file.el")
    (insert "test content")
    (set-buffer-modified-p t)
    (let* ((result (claude-code-ide-mcp-handle-check-document-dirty '((filePath . "/tmp/test-file.el")))))
    (should (eq (alist-get 'isDirty result) t)))) (with-temp-buffer
    (setq buffer-file-name "/tmp/test-file2.el")
    (insert "test content")
    (set-buffer-modified-p nil)
    (let* ((result (claude-code-ide-mcp-handle-check-document-dirty '((filePath . "/tmp/test-file2.el")))))
    (should (eq (alist-get 'isDirty result) :json-false)))) (let* ((result (claude-code-ide-mcp-handle-check-document-dirty '((filePath . "/tmp/non-existent-file.el")))))
    (should (eq (alist-get 'isDirty result) :json-false))) (should-error (claude-code-ide-mcp-handle-check-document-dirty '()) :type 'mcp-error))

(defvar claude-code-ide-mcp-server-tests--mock-server-started)

(defvar claude-code-ide-mcp-server-tests--mock-server-port 12345)

(defun claude-code-ide-tests---mock-server-start (&optional _port)
...
(defun claude-code-ide-tests---mock-server-stop (_process)
  "Mock server stop function."
  (setq claude-code-ide-mcp-server-tests--mock-server-started nil))

(defvar claude-code-ide-mcp-server-tests--last-response)

(defvar claude-code-ide-mcp-server-tests--last-response-headers)

(defvar claude-code-ide-mcp-server-tests--last-response-status)

(cl-defstruct claude-code-ide-mcp-server-tests--mock-request process headers body)

(cl-defstruct claude-code-ide-mcp-server-tests--mock-process)

(ert-deftest claude-code-ide-mcp-server-test-session-lifecycle (nil) "Test MCP tools server session lifecycle." (let* ((claude-code-ide-enable-mcp-server t)
        (claude-code-ide-mcp-server--session-count 0)
        (claude-code-ide-mcp-server--server nil)
        (claude-code-ide-mcp-server--port nil))
    (cl-letf (((symbol-function 'claude-code-ide-mcp-http-server-start) #'claude-code-ide-mcp-server-tests--mock-server-start) ((symbol-function 'claude-code-ide-mcp-http-server-stop) #'claude-code-ide-mcp-server-tests--mock-server-stop) ((symbol-function 'require) (lambda (feature &optional _filename _noerror)
    (cond
  ((eq feature 'claude-code-ide-mcp-http-server) nil)
  ((memq feature '(claude-code-ide-mcp-server websocket vterm flycheck claude-code-ide-debug claude-code-ide-mcp-handlers claude-code-ide transient)) nil)
  (t (funcall (cl-letf-saved-symbol-function 'require) feature _filename _noerror)))))) (claude-code-ide-mcp-server-session-started) (should (equal claude-code-ide-mcp-server--session-count 1)) (setq claude-code-ide-mcp-server--server (car (claude-code-ide-mcp-server-tests--mock-server-start))) (setq claude-code-ide-mcp-server--port (cdr (claude-code-ide-mcp-server-tests--mock-server-start))) (should claude-code-ide-mcp-server--server) (should (equal claude-code-ide-mcp-server--port claude-code-ide-mcp-server-tests--mock-server-port)) (claude-code-ide-mcp-server-session-started) (should (equal claude-code-ide-mcp-server--session-count 2)) (claude-code-ide-mcp-server-session-ended) (should (equal claude-code-ide-mcp-server--session-count 1)) (should claude-code-ide-mcp-server--server) (claude-code-ide-mcp-server-session-ended) (should (equal claude-code-ide-mcp-server--session-count 0)) (claude-code-ide-mcp-server-tests--mock-server-stop claude-code-ide-mcp-server--server) (setq claude-code-ide-mcp-server--server nil) (setq claude-code-ide-mcp-server--port nil) (should-not claude-code-ide-mcp-server--server) (should-not claude-code-ide-mcp-server--port))))

(ert-deftest claude-code-ide-mcp-server-test-config-generation (nil) "Test MCP configuration generation." (let* ((claude-code-ide-enable-mcp-server t)
        (claude-code-ide-mcp-server--server 'mock-server)
        (claude-code-ide-mcp-server--port 8080))
    (cl-letf (((symbol-function 'process-live-p) (lambda (_)
    t)) ((symbol-function 'ws-process) (lambda (_)
    'mock-process))) (let* ((config (claude-code-ide-mcp-server-get-config)))
    (should config)
    (should (equal (alist-get 'type (alist-get 'emacs-tools (alist-get 'mcpServers config))) "http"))
    (should (equal (alist-get 'url (alist-get 'emacs-tools (alist-get 'mcpServers config))) "http://localhost:8080/mcp"))))
    (let* ((claude-code-ide-mcp-server--server nil)
        (claude-code-ide-mcp-server--port nil)
        (config (claude-code-ide-mcp-server-get-config)))
    (should-not config))))

(ert-deftest claude-code-ide-mcp-server-test-disabled (nil) "Test that MCP tools server does nothing when disabled." (let* ((claude-code-ide-enable-mcp-server nil)
        (claude-code-ide-mcp-server--session-count 0))
    (should-not (claude-code-ide-mcp-server-ensure-server))
    (claude-code-ide-mcp-server-session-started)
    (should (equal claude-code-ide-mcp-server--session-count 1))
    (should-not claude-code-ide-mcp-server--server)))

(ert-deftest claude-code-ide-mcp-server-test-tool-config (nil) "Test tool configuration structure." (let* ((claude-code-ide-mcp-server-tools '((test-function :description "Test function" :parameters ((:name "arg1" :type "string" :required t) (:name "arg2" :type "number" :required nil))))))
    ...
    (should (equal (length (plist-get plist :parameters)) 2)))))

(ert-deftest claude-code-ide-mcp-server-test-json-encoding (nil) "Test JSON encoding of MCP config." (let* ((config '((mcpServers . ((emacs-tools . ((transport . "http") (url . "http://localhost:8080/mcp"))))))))
...
(ert-deftest claude-code-ide-mcp-server-test-session-registration (nil) "Test session registration and retrieval." (let* ((session-id "test-session-123")
...
(ert-deftest claude-code-ide-mcp-server-test-session-lifecycle-detailed (nil) "Test complete session lifecycle with detailed tracking." (let* ((session-id "test-session-789")
  ...
  (should (equal claude-code-ide-mcp-server--session-count 0))
  (should-not (gethash session-id claude-code-ide-mcp-server--sessions)))
  (kill-buffer buffer)
  (setq claude-code-ide-mcp-server--session-count 0)
  (clrhash claude-code-ide-mcp-server--sessions))))

(ert-deftest claude-code-ide-mcp-server-test-config-with-session-id (nil) "Test MCP config generation with session ID." (cl-letf (((symbol-function 'claude-code-ide-mcp-server-get-port) (lambda ()
    12345))) (let* ((config (claude-code-ide-mcp-server-get-config)))
    (should config)
    (let* ((url (alist-get 'url (alist-get 'emacs-tools (alist-get 'mcpServers config)))))
    (should (equal url "http://localhost:12345/mcp")))) (let* ((config (claude-code-ide-mcp-server-get-config "my-session-123")))
    (should config)
...
(ert-deftest claude-code-ide-emacs-tools-test-imenu-list-symbols (nil) "Test the imenu-list-symbols MCP tool." (require 'claude-code-ide-emacs-tools) (let* ((test-file (make-temp-file "test-imenu-" nil ".el"))
        (session-id "test-session-imenu")
        (project-dir (temporary-file-directory)))
    (unwind-protect
    (progn
  (with-temp-file test-file (insert ";;; Test file for imenu\n\n" "(defun test-function-1 (arg)\n" "  \"A test function.\"\n" "  (message \"Hello %s\" arg))\n\n" "(defvar test-variable 42\n" "  \"A test variable.\")\n\n" "(defun test-function-2 ()\n" "  \"Another test function.\"\n" "  (+ 1 2))\n\n" "(defconst test-constant 'foo\n" "  \"A test constant.\")\n"))
  ...
  (should-error (claude-code-ide-mcp-imenu-list-symbols nil) :type 'error))
