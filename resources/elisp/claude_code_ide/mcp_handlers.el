...
(defun claude-code-ide-mcp-handlers---find-session-for-file (file-path)
  ...
  (let* ((expanded-file (expand-file-name file-path))
    ...
    (throw 'found t))) claude-code-ide-mcp--sessions))
    found-session))
...
(defun claude-code-ide-mcp-handlers---handle-ediff-startup (tab-name session saved-winconf startup-hook-fn)
  ...
  (error nil)))))))
    ...
    (cond
  ((and claude-code-ide-focus-claude-after-ediff claude-window) (select-window claude-window))
  (t (select-window original-window))))
...
(defun claude-code-ide-mcp-handlers-handle-open-file (arguments)
  ...
  (cond
  ((and start-text end-text) (progn
  (goto-char (point-min))
  (if (search-forward start-text nil t) (let* ((start-pos (match-beginning 0)))
  ...
  (when end-line
    ...
    (end-of-line)
    (activate-mark)))))
  (list (list (cons 'type "text") (cons 'text "FILE_OPENED"))))
...
(defun claude-code-ide-mcp-handlers-handle-get-current-selection (_arguments)
  ...
  (let* ((file-path (or (buffer-file-name) ""))
    ...
    (goto-char end)
    (1+ (current-column)))))
    (append (list (cons 'text text) (cons 'filePath file-path)) (when file-url
    (list (cons 'fileUrl file-url))) (list (cons 'selection (list (cons 'start (list (cons 'line start-line) (cons 'character start-col))) (cons 'end (list (cons 'line end-line) (cons 'character end-col))) (cons 'isEmpty :json-false)))))) (let* ((cursor-line (line-number-at-pos))
        (cursor-col (1+ (current-column))))
    (append (list (cons 'text "") (cons 'filePath file-path)) (when file-url
    (list (cons 'fileUrl file-url))) (list (cons 'selection (list (cons 'start (list (cons 'line cursor-line) (cons 'character cursor-col))) (cons 'end (list (cons 'line cursor-line) (cons 'character cursor-col))) (cons 'isEmpty t)))))))))
...
(defun claude-code-ide-mcp-handlers-handle-get-open-editors (_arguments)
  ...
  (let* ((editors '())
    ...
    (push (list (cons 'path file) (cons 'name (buffer-name buffer)) (cons 'active (eq buffer (current-buffer))) (cons 'isDirty (if (buffer-modified-p buffer) t :json-false)) (cons 'fileUrl (concat "file://" file))) editors))))
    (list (cons 'editors (vconcat (nreverse editors))))))
...
(defun claude-code-ide-mcp-handlers-handle-get-workspace-folders (_arguments)
  ...
  (let* ((project-dir (or (claude-code-ide-mcp--get-buffer-project) default-directory)))
    (list (cons 'folders (vconcat (list (expand-file-name project-dir)))))))
...
(defun claude-code-ide-mcp-handlers-handle-save-document (arguments)
  ...
  (let* ((path (alist-get 'path arguments)))
    ...
    (save-buffer)
    (list (list (cons 'type "text") (cons 'text "DOCUMENT_SAVED")))) (signal 'mcp-error (list (format "No buffer visiting %s" path)))))
...
(defun claude-code-ide-mcp-handlers-handle-close-tab (arguments)
  ...
  (path (let* ((buffer (find-buffer-visiting path)))
    (if buffer (progn
  (kill-buffer buffer)
  (list (list (cons 'type "text") (cons 'text "TAB_CLOSED")))) (signal 'mcp-error (list (format "No buffer visiting %s" path))))))
  (tab-name (let* ((found-session nil)
        (found-diff-info nil))
  ...
  (list (list (cons 'type "text") (cons 'text "TAB_CLOSED")))) (let* ((buffer (get-buffer tab-name)))
    (if buffer (progn
  (kill-buffer buffer)
  (list (list (cons 'type "text") (cons 'text "TAB_CLOSED")))) (signal 'mcp-error (list (format "No buffer named %s" tab-name))))))))
  (t (signal 'mcp-error '("Either 'path' or 'tab_name' must be provided"))))))
...
(defun claude-code-ide-mcp-handlers-handle-open-diff (arguments)
  ...
  (let* ((old-file-path (alist-get 'old_file_path arguments))
    ...
    (let* ((active-diffs (claude-code-ide-mcp-handlers--get-active-diffs session)))
    (puthash tab-name (list (cons 'buffer-A buffer-A) (cons 'buffer-B buffer-B) (cons 'old-file-path old-file-path) (cons 'new-file-path new-file-path) (cons 'file-exists file-exists) (cons 'saved-winconf saved-winconf) (cons 'session session) (cons 'created-at (current-time))) active-diffs))
  ...
  (error (when buffer-B
    ...
    (list (cons 'deferred t) (cons 'unique-key tab-name) (cons 'session session))))))
...
(defun claude-code-ide-mcp-handlers-handle-close-all-diff-tabs (_arguments)
  ...
  (let* ((closed-count 0)
    ...
    (list (list (cons 'type "text") (cons 'text (format "CLOSED_%d_DIFF_TABS" closed-count))))))
...
(defun claude-code-ide-mcp-handlers-handle-check-document-dirty (arguments)
  ...
  (let* ((path (alist-get 'filePath arguments)))
    ...
    (let* ((buffer (find-buffer-visiting path)))
    (if buffer (list (cons 'isDirty (if (buffer-modified-p buffer) t :json-false))) (list (cons 'isDirty :json-false))))))

(defun claude-code-ide-mcp-handlers---build-tool-list ()
  "Build the tool list, conditionally including ediff tools."
  (append (list '("openFile" . claude-code-ide-mcp-handlers-handle-open-file) '("getCurrentSelection" . claude-code-ide-mcp-handlers-handle-get-current-selection) '("getOpenEditors" . claude-code-ide-mcp-handlers-handle-get-open-editors) '("getWorkspaceFolders" . claude-code-ide-mcp-handlers-handle-get-workspace-folders) '("getDiagnostics" . claude-code-ide-mcp-handlers-handle-get-diagnostics) '("saveDocument" . claude-code-ide-mcp-handlers-handle-save-document) '("close_tab" . claude-code-ide-mcp-handlers-handle-close-tab)) (when (bound-and-true-p claude-code-ide-use-ide-diff)
    '(("openDiff" . claude-code-ide-mcp-handlers-handle-open-diff) ("closeAllDiffTabs" . claude-code-ide-mcp-handlers-handle-close-all-diff-tabs))) (list '("checkDocumentDirty" . claude-code-ide-mcp-handlers-handle-check-document-dirty))))
...
(defun claude-code-ide-mcp-handlers---build-tool-schemas ()
  ...
  (append '(("openFile" . ((type . "object") (properties . ((path . ((type . "string") (description . "Path to the file to open"))) (startLine . ((type . "integer") (description . "Start line for selection"))) (endLine . ((type . "integer") (description . "End line for selection"))) (startText . ((type . "string") (description . "Start text pattern for selection (takes precedence over line numbers)"))) (endText . ((type . "string") (description . "End text pattern for selection"))))) (required . ["path"]))) ("getCurrentSelection" . ((type . "object") (properties . :json-empty))) ("getOpenEditors" . ((type . "object") (properties . :json-empty))) ("getWorkspaceFolders" . ((type . "object") (properties . :json-empty))) ("getDiagnostics" . ((type . "object") (properties . ((uri . ((type . "string") (description . "Optional file URI to get diagnostics for. If not provided, gets diagnostics for all files."))))) (required . []))) ("saveDocument" . ((type . "object") (properties . ((path . ((type . "string") (description . "Path to the file to save"))))) (required . ["path"]))) ("close_tab" . ((type . "object") (properties . ((path . ((type . "string") (description . "Path to the file to close"))) (tab_name . ((type . "string") (description . "Name of the tab to close"))))) (required . [])))) (when (bound-and-true-p claude-code-ide-use-ide-diff)
    '(("openDiff" . ((type . "object") (properties . ((old_file_path . ((type . "string") (description . "Path to the original file"))) (new_file_path . ((type . "string") (description . "Path to the new file (usually same as old)"))) (new_file_contents . ((type . "string") (description . "New content to diff against"))) (tab_name . ((type . "string") (description . "Name for the diff tab"))))) (required . ["old_file_path" "new_file_path" "new_file_contents" "tab_name"]))) ("closeAllDiffTabs" . ((type . "object") (properties . :json-empty))))) '(("checkDocumentDirty" . ((type . "object") (properties . ((filePath . ((type . "string") (description . "Path to the file to check"))))) (required . ["filePath"]))))))
...
(defun claude-code-ide-mcp-handlers---build-tool-descriptions ()
  ...
  (append '(("openFile" . "Open a file in the editor and optionally select a range of text") ("getCurrentSelection" . "Get the currently selected text and its location") ("getOpenEditors" . "Get the list of currently open editors/buffers") ("getWorkspaceFolders" . "Get the current workspace/project folders") ("getDiagnostics" . "Get language diagnostics from Emacs") ("saveDocument" . "Save a document to disk") ("close_tab" . "Close a tab/buffer")) (when (bound-and-true-p claude-code-ide-use-ide-diff)
    '(("openDiff" . "Open a diff view comparing old and new file contents") ("closeAllDiffTabs" . "Close all open diff tabs in the current session"))) '(("checkDocumentDirty" . "Check if a document has unsaved changes"))))
