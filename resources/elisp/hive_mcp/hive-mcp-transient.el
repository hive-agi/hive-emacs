;;; hive-mcp-transient.el --- -*- lexical-binding: t; -*-
...
(defun hive-mcp-transient--status-line ()
  ...
  (let* ((init (if (bound-and-true-p hive-mcp--initialized) "initialized" "not initialized"))
        ...
        (channel (if (and (fboundp 'hive-mcp-channel-connected-p) (hive-mcp-channel-connected-p)) "connected" "disconnected")))
    (format "hive-mcp [mode: %s | %s | channel: %s]" (propertize mode 'face (if (bound-and-true-p hive-mcp-mode) 'success 'warning)) (propertize init 'face (if (bound-and-true-p hive-mcp--initialized) 'success 'warning)) (propertize channel 'face (if (and (fboundp 'hive-mcp-channel-connected-p) (hive-mcp-channel-connected-p)) 'success 'transient-inactive-value)))))

(transient-define-prefix hive-mcp-transient-main ()
  "Hive MCP main menu."
  [:description hive-mcp-transient--status-line]
  ["Hive MCP" ["Memory" ("n" "Add note" hive-mcp-add-note-interactive) ("s" "Add snippet" hive-mcp-add-snippet-interactive) ("c" "Add convention" hive-mcp-add-convention-interactive) ("d" "Add decision" hive-mcp-add-decision-interactive) ("l" "Show memory" hive-mcp-show-memory)] ["Workflows" ("w" "Run workflow" hive-mcp-run-workflow-interactive) ("W" "Wrap session" hive-mcp-workflows-workflow-wrap) ("C" "Catchup" hive-mcp-workflows-workflow-catchup)] ["Context & History" ("x" "Show context" hive-mcp-show-context) ("h" "Conversation history" hive-mcp-show-conversation-history) ("H" "Clear conversation" hive-mcp-clear-conversation)] ["Submenus" ("S" "Swarm / Hivemind" hive-mcp-transient-hivemind-menu) ("N" "Connection" hive-mcp-transient-connection-menu) ("K" "Kanban" hive-mcp-transient-kanban-menu)]])

(transient-define-prefix hive-mcp-transient-hivemind-menu ()
  "Hivemind swarm menu."
  ["Hivemind" ("s" "Agent status" hive-mcp-hivemind-status) ("l" "Show log" hive-mcp-hivemind-show-log) ("c" "Clear state" hive-mcp-hivemind-clear)])

(transient-define-prefix hive-mcp-transient-connection-menu ()
  "Connection management menu."
  ["Connection" ("c" "Connect channel" hive-mcp-channel-connect) ("d" "Disconnect channel" hive-mcp-channel-disconnect) ("i" "Initialize hive-mcp" hive-mcp-initialize) ("r" "Reset hive-mcp" hive-mcp-reset)])

(transient-define-prefix hive-mcp-transient-kanban-menu ()
  "Kanban board menu."
  ["Kanban" ("b" "Open board" hive-mcp-kanban-board)])
