...
(let* ((this-dir (file-name-directory (or load-file-name buffer-file-name))))
    (add-to-list 'load-path (expand-file-name "swarm" this-dir)))
(require 'cl-lib)
(require 'json)
(require 'subr-x)
...
(defun hive-mcp-swarm---format-slave-detail (id slave)
  ...
  (list (cons 'slave-id id) (cons 'name (plist-get slave :name)) (cons 'status (hive-mcp-swarm---get-slave-status-name slave)) (cons 'depth (plist-get slave :depth)) (cons 'parent-id (plist-get slave :parent-id)) (cons 'cwd (plist-get slave :cwd)) (cons 'current-task (plist-get slave :current-task)) (cons 'tasks-completed (plist-get slave :tasks-completed))))
...
(defun hive-mcp-swarm---format-ling-data (id slave)
  ...
  (list (cons 'slave-id id) (cons 'name (plist-get slave :name)) (cons 'presets (hive-mcp-swarm---get-presets-or-empty slave)) (cons 'cwd (plist-get slave :cwd)) (cons 'status (hive-mcp-swarm---get-slave-status-name slave))))
...
(defun hive-mcp-swarm---format-orphan-detail (orphan)
  ...
  (list (cons 'slave-id (plist-get orphan :slave-id)) (cons 'name (plist-get orphan :name)) (cons 'status "orphan") (cons 'depth nil) (cons 'parent-id nil) (cons 'current-task nil) (cons 'tasks-completed nil) (cons 'orphan t) (cons 'discovered-via "buffer-introspection")))

(defun hive-mcp-swarm---format-orphan-ling (orphan)
  "Format ORPHAN plist as alist for lings list API response."
  (list (cons 'slave-id (plist-get orphan :slave-id)) (cons 'name (plist-get orphan :name)) (cons 'presets (list )) (cons 'cwd nil) (cons 'status "orphan") (cons 'orphan t)))
...
(defun hive-mcp-swarm---format-pending-prompt (prompt)
  ...
  (list :slave-id (plist-get prompt :slave-id) :prompt (plist-get prompt :prompt) :timestamp (format-time-string "%Y-%m-%dT%H:%M:%S" (plist-get prompt :timestamp))))
...
(defun hive-mcp-swarm---build-status-response (total idle working error-count orphan-count slaves-detail)
  ...
  (list :session-id hive-mcp-swarm--session-id :status (if (> total 0) "active" "inactive") :current-depth hive-mcp-swarm--current-depth :safeguards (list :max-depth hive-mcp-swarm-max-depth :max-slaves hive-mcp-swarm-max-slaves :rate-limit (list :window-seconds hive-mcp-swarm-rate-limit-window :max-spawns hive-mcp-swarm-rate-limit-max-spawns :recent-spawns (length hive-mcp-swarm--spawn-timestamps))) :slaves (list :total total :idle idle :working working :error error-count :orphan orphan-count) :tasks (list :total (hash-table-count hive-mcp-swarm--tasks)) :slaves-detail slaves-detail))
...
(defun hive-mcp-swarm-api-spawn (ling-name presets &optional cwd terminal kanban-task-id context-file)
  ...
  (let* ((valid-cwd (when (stringp cwd)
    ...
    (condition-case err-150
    (hive-mcp-swarm-spawn ling-name :presets presets :cwd valid-cwd :terminal (when (and terminal (stringp terminal))
    (intern terminal)) :kanban-task-id valid-kanban-id :injected-context injected-context)
  (error (hive-mcp-graceful--log-error "fallback triggered" err-150)
      (list :error "spawn-failed" :name ling-name :reason "unknown")))))
...
(defun hive-mcp-swarm-api-dispatch (slave-id prompt &optional timeout-ms)
  ...
  (condition-case err-151
    (hive-mcp-swarm-dispatch slave-id prompt :timeout timeout-ms)
  (error (hive-mcp-graceful--log-error "fallback triggered" err-151)
      (list :status "error" :error "dispatch-failed" :slave-id slave-id :reason "unknown"))))
...
(defun hive-mcp-swarm-api-collect (task-id &optional timeout-ms)
  ...
  (condition-case err-152
    ...
    (cond
  ((not task) (list :task-id task-id :status "error" :error "Task not found"))
  ((eq (plist-get task :status) 'completed) (list :task-id task-id :status "completed" :result (plist-get task :result)))
  ((eq (plist-get task :status) 'timeout) (list :task-id task-id :status "timeout" :error (plist-get task :error)))
  ((and elapsed-ms (> elapsed-ms task-timeout)) (progn
  ...
  (when slave
    ...
    (plist-put slave :current-task nil)
    (plist-put slave :tasks-failed (1+ (or (plist-get slave :tasks-failed) 0))))
  (hive-mcp-swarm--emit-task-failed task-id slave-id (format "Timed out after %dms" elapsed-ms))
  (list :task-id task-id :status "timeout" :error (format "Timed out after %dms" elapsed-ms) :elapsed-ms elapsed-ms)))
  (t (if-let* ((result (hive-mcp-swarm--check-task-completion task-id)))
    (progn
  ...
  (when slave
    (plist-put slave :status 'idle)
    (plist-put slave :current-task nil)
    (plist-put slave :tasks-completed (1+ (or (plist-get slave :tasks-completed) 0))))
  (hive-mcp-swarm--emit-task-completed task-id slave-id result)
  (list :task-id task-id :status "completed" :result result :elapsed-ms elapsed-ms))
  (list :task-id task-id :status "polling" :elapsed-ms elapsed-ms :timeout-ms task-timeout :message "Task still running, poll again")))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-152)
      (list :task-id task-id :status "error" :result nil :error "collection-failed"))))
...
(defun hive-mcp-swarm-api-kill (slave-id)
  ...
  (condition-case err-153
    (if (hive-mcp-swarm-kill slave-id) (list :killed slave-id :success t) (list :error "kill-blocked" :slave-id slave-id :reason "buffer-safety-validation-failed" :message "Buffer did not pass safety checks (may have been coordinator or invalid target)"))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-153)
      (list :error "kill-failed" :slave-id slave-id))))

(defun hive-mcp-swarm-api-kill-all ()
  "API: Kill all slaves.\nReturns result plist. Never fails."
  (condition-case err-154
    (let* ((count (hash-table-count hive-mcp-swarm--slaves)))
    (hive-mcp-swarm-kill-all)
    (list :killed-count count))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-154)
      (list :error "kill-all-failed" :killed-count 0))))
...
(defun hive-mcp-swarm-api-pending-prompts ()
  ...
  (condition-case err-155
    (let* ((pending (hive-mcp-swarm-prompts-get-pending))
        (prompts (mapcar #'-format-pending-prompt pending)))
    (list :count (length prompts) :prompts prompts :mode hive-mcp-swarm-prompts-mode))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-155)
      (list :error "pending-prompts-failed" :count 0 :prompts nil))))
...
(defun hive-mcp-swarm-api-respond-prompt (slave-id response)
  ...
  (condition-case err-156
    (if (hive-mcp-swarm-prompts-respond-to slave-id response) (list :success t :slave-id slave-id :response response) (list :success nil :error "no-pending-prompt" :slave-id slave-id))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-156)
      (list :error "respond-prompt-failed" :slave-id slave-id))))
