...
(defun hive-mcp-workflows-workflow-run (&rest args)
  ...
  (let* ((result (hive-mcp-workflows---execute spec args)))
...
(defun hive-mcp-workflows---step-prompt (step env)
  ...
  (let* ((msg (plist-get step :message))
        ...
        (result (clel-read-string (concat msg ": ") default)))
...
(defun hive-mcp-workflows--wrap--merge-args (gathered provided)
  ...
  (let* ((result (copy-sequence provided)))
    ...
    (if (> (length content) 100) (concat (substring content 0 97) "...") content))) (seq-take notes 5)))))
...
(defalias 'hive-mcp-workflow--load #'hive-mcp-workflows---load)

(defalias 'hive-mcp-workflow--register-builtins #'hive-mcp-workflows---register-builtins)

(defalias 'hive-mcp-workflow-run-interactive #'hive-mcp-workflows-workflow-run-interactive)
