...
(defun hive-mcp-swarm-tasks---slave-ready-p (slave-id)
  ...
  (when-let* ((slave (gethash slave-id hive-mcp-swarm--slaves))
    ...
    (and (buffer-live-p buffer) (hive-mcp-swarm-terminal-ready-p buffer))))
...
(defun hive-mcp-swarm-tasks---process-queue-entry (entry)
  ...
  (let* ((slave-id (plist-get entry :slave-id))
    ...
    (cond
  ((hive-mcp-swarm-tasks---slave-truly-dead-p slave) (progn
  (message "[swarm-tasks] Queue: %s slave dead, dropping dispatch" slave-id)
  (hive-mcp-swarm-tasks---notify-dispatch-dropped slave-id "slave-dead" prompt retries queued-at)
  :failed))
  ((>= retries hive-mcp-swarm-dispatch-max-retries) (progn
  (message "[swarm-tasks] Queue: %s max retries (%d) exceeded, dropping" slave-id retries)
  (hive-mcp-swarm-tasks---notify-dispatch-dropped slave-id "max-retries-exceeded" prompt retries queued-at)
  :failed))
