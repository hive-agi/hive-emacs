...
(defun hive-mcp-swarm-slaves-generate-id (ling-name)
  "Generate unique slave ID for LING-NAME."
  (format "swarm-%s-%d" ling-name (floor (float-time))))
...
(cl-defun hive-mcp-swarm-slaves-spawn (ling-name &key presets cwd role terminal backend model kanban-task-id injected-context)
  ...
  (let* ((slave-id (hive-mcp-swarm-slaves-generate-id ling-name))
    ...
    (puthash slave-id (list :slave-id slave-id :name ling-name :role role :presets presets :status 'spawning :buffer nil :terminal term-backend :backend term-backend :model model :cwd work-dir :depth spawn-depth :parent-id parent-id :kanban-task-id kanban-task-id :context-injected (not (null injected-context)) :current-task nil :task-queue '() :tasks-completed 0 :tasks-failed 0 :spawned-at (format-time-string "%FT%T%z") :last-activity (format-time-string "%FT%T%z")) hive-mcp-swarm--slaves)
    ...
    (hive-mcp-swarm-events-emit-slave-spawned slave-id ling-name presets work-dir kanban-task-id)
    (let* ((ctx injected-context))
    (run-with-timer 0 nil (lambda ()
    (condition-case err
    (hive-mcp-swarm-slaves---do-spawn-async slave-id ling-name presets work-dir term-backend ctx)
  (error (when-let* ((slave (gethash slave-id hive-mcp-swarm--slaves)))
...
(defun hive-mcp-swarm-slaves---do-spawn-async (slave-id ling-name presets work-dir term-backend &optional injected-context)
  ...
  (let* ((slave (gethash slave-id hive-mcp-swarm--slaves))
        (buffer-name (format "%s%s*" hive-mcp-swarm-buffer-prefix ling-name))
        ...
        (system-prompt (if base-prompt (concat identity-section base-prompt) identity-section))
        (buffer nil))
    (unless slave
  ...
  ((quote eat) (with-current-buffer buffer
    (eat-mode)
    (eat-exec buffer "swarm-shell" "/bin/bash" nil '("-l"))) (run-at-time 0.5 nil (lambda ()
    (condition-case err
    (when (buffer-live-p buffer)
    (with-current-buffer buffer
  ...
  ((quote ollama) (when (buffer-live-p buffer)
    ...
    (hive-mcp-ellama-swarm-spawn slave-id ling-name model)
...
(defun hive-mcp-swarm-slaves---valid-kill-target-p (buffer slave-id)
  ...
  (when (buffer-live-p buffer)
    ...
    (cond
  ((not (string-prefix-p hive-mcp-swarm-buffer-prefix buf-name)) (progn
  (message "[swarm-kill] BLOCKED: Buffer '%s' lacks swarm prefix for slave %s" buf-name slave-id)
  nil))
  ((string-match-p "coordinator" buf-name-lower) (progn
  (message "[swarm-kill] BLOCKED: Buffer '%s' contains 'coordinator' - refusing to kill" buf-name)
  nil))
  ((or (string-match-p "\\*claude-code" buf-name-lower) (string-match-p "\\*claude code" buf-name-lower) (string-match-p "master" buf-name-lower)) (progn
  (message "[swarm-kill] BLOCKED: Buffer '%s' matches coordinator pattern - refusing to kill" buf-name)
  nil))
  ((and slave-name (not (string-match-p (regexp-quote slave-name) buf-name))) (progn
  (message "[swarm-kill] BLOCKED: Buffer '%s' doesn't match slave name '%s' from ID %s" buf-name slave-name slave-id)
  (message "[swarm-kill] This indicates registry corruption - buffer/slave-id mismatch!")
  nil))
  (t (progn
  (message "[swarm-kill] Validated: killing buffer '%s' for slave %s (name: %s)" buf-name slave-id (or slave-name "unknown"))
  t))))))
...
(defun hive-mcp-swarm-slaves-kill (slave-id)
  ...
  t))
  ((not (buffer-live-p buffer)) (progn
  (message "[swarm-kill] Info: slave %s buffer already dead, cleaning registry" slave-id)
  (hive-mcp-swarm-events-emit-slave-killed slave-id)
  (remhash slave-id hive-mcp-swarm--slaves)
  (hive-mcp-swarm-prompts-clear-slave slave-id)
  t))
  ((not (hive-mcp-swarm-slaves---valid-kill-target-p buffer slave-id)) (progn
  (message "[swarm-kill] BLOCKED: Buffer '%s' failed safety validation for slave %s" (buffer-name buffer) slave-id)
