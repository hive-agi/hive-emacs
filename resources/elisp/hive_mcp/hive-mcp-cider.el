...
(defcustom hive-mcp-cider-cljs-repl-type "shadow"
  "ClojureScript REPL type for cider-connect-cljs."
  :group 'hive-mcp-cider
  :type '(choice (const "shadow") (const "figwheel") (const "browser")))

(defcustom hive-mcp-cider-shadow-build ":app"
  "Default shadow-cljs build ID."
  :group 'hive-mcp-cider
  :type 'string)

(defcustom hive-mcp-cider-cljel-project-dir "~/PP/clojure-elisp"
...
(defun hive-mcp-cider--session-nrepl-command (repl-type port dir)
  ...
  (let* ((port-str (number-to-string port)))
    (pcase repl-type
  ((quote cljs) (list "npx" "shadow-cljs" "watch" hive-mcp-cider-shadow-build))
...
(defun hive-mcp-cider-spawn-session (name &optional repl-type port project-dir agent-id)
  ...
  (let* ((rtype (or repl-type 'clj))
        (the-port (or port (hive-mcp-cider--find-available-port)))
        (dir (or project-dir (if (eq rtype 'cljel) (expand-file-name hive-mcp-cider-cljel-project-dir) (hive-mcp-cider--project-dir))))
        (default-directory dir)
        (buf-name (format "*nREPL-%s*" name))
        (cmd (hive-mcp-cider--session-nrepl-command rtype the-port dir))
        (process (apply #'start-process (format "nrepl-%s" name) buf-name cmd))
  ...
  (error (message "[cider] Timer error connecting session %s: %s" name (error-message-string err))))))))
    (puthash name (list :port the-port :process process :buffer buf-name :agent-id agent-id :project-dir dir :repl-type rtype :status 'starting :cider-buffer nil :timer timer) hive-mcp-cider--sessions)
...
(defun hive-mcp-cider-connect-session (name host port &optional repl-type agent-id)
  ...
  (let* ((host (or host "localhost"))
        ...
        (conn (hive-mcp-cider--connect-by-repl-type rtype port))
        (cider-buf (buffer-name conn)))
    (puthash name (list :port port :process nil :buffer nil :agent-id agent-id :project-dir nil :repl-type rtype :status 'connected :cider-buffer cider-buf :timer nil) hive-mcp-cider--sessions)
...
(defun hive-mcp-cider--connect-by-repl-type (repl-type port)
  "Connect CIDER to nREPL on PORT using the appropriate method for REPL-TYPE.\nReturns the connection buffer.\nFor cljs and cljel, connects as CLJ first (non-blocking), then upgrades\nasynchronously via timer to avoid blocking Emacs."
  (pcase repl-type
  ((quote cljs) (let* ((conn (cider-connect-clj (list :host "localhost" :port port)))
        (old-name (buffer-name conn))
        (select-form (format "(do (require '[shadow.cljs.devtools.api :as shadow]) (shadow/nrepl-select %s))" hive-mcp-cider-shadow-build)))
    ...
    (hive-mcp-cider--update-session-buffer-name conn))))))))))
    conn))
  ((quote cljel) (let* ((conn (cider-connect-clj (list :host "localhost" :port port))))
    (run-at-time 1.0 nil (lambda ()
    (when (and (buffer-live-p conn) (fboundp 'cider-cljel-start))
    (with-current-buffer conn
    (cider-cljel-start)))))
    conn))
  ('_ (cider-connect-clj (list :host "localhost" :port port)))))

(defun hive-mcp-cider--update-session-buffer-name (conn)
  ...
  (let* ((new-name (when (buffer-live-p conn)
    ...
    (puthash name (plist-put session :cider-buffer new-name) hive-mcp-cider--sessions)
    (message "hive-mcp-cider: Session '%s' buffer updated to %s" name new-name)))) hive-mcp-cider--sessions))))

(defun hive-mcp-cider--try-connect-session (name)
  "Try to connect CIDER to session NAME.\nDispatches connection method based on session :repl-type."
  (let* ((session (gethash name hive-mcp-cider--sessions))
    ...
    (let* ((conn (hive-mcp-cider--connect-by-repl-type repl-type port)))
...
(defun hive-mcp-cider-list-sessions ()
  ...
  (let* ((sessions '()))
    ...
    (push (list :name name :port (plist-get props :port) :repl-type (if (symbolp rtype) (symbol-name rtype) (or rtype "clj")) :status (if (symbolp status) (symbol-name status) status) :agent-id (plist-get props :agent-id) :cider-buffer (plist-get props :cider-buffer)) sessions))) hive-mcp-cider--sessions)
...
(defun hive-mcp-cider-eval-in-session (name code &optional timeout)
  ...
  (let* ((session (gethash name hive-mcp-cider--sessions))
    ...
    (with-current-buffer cider-buf
    (hive-mcp-cider--eval-with-heartbeat code timeout))))
...
(defun hive-mcp-cider--ensure-connected ()
  ...
  ((hive-mcp-cider--port-open-p hive-mcp-cider-nrepl-port) (progn
  ...
  t)
  (error nil))))
  (t (let* ((auto-session (gethash "auto" hive-mcp-cider--sessions)))
...
(defun hive-mcp-cider--auto-connect-tick ()
  ...
  ((hive-mcp-cider--try-connect) (hive-mcp-cider--stop-auto-connect))
  ((and (> hive-mcp-cider-connect-max-retries 0) (>= hive-mcp-cider--connect-attempts hive-mcp-cider-connect-max-retries)) (progn
  (hive-mcp-cider--stop-auto-connect)
  (message "hive-mcp-cider: Auto-connect gave up after %d attempts" hive-mcp-cider--connect-attempts)))))
...
(defun hive-mcp-cider--start-auto-connect ()
  ...
  (unless hive-mcp-cider--connect-timer
    ...
    (setq hive-mcp-cider--connect-timer (run-with-timer hive-mcp-cider-connect-retry-interval hive-mcp-cider-connect-retry-interval #'hive-mcp-cider--auto-connect-tick))
...
(transient-define-prefix hive-mcp-cider-transient ()
  ...
  ["hive-mcp + CIDER" ["nREPL Server" ("n" "Start nREPL (async)" hive-mcp-cider-start-nrepl) ("N" "Stop nREPL" hive-mcp-cider-stop-nrepl) ("c" "Auto-connect" hive-mcp-cider-auto-connect)] ["Spawn" ("sc" "Spawn CLJ" (lambda ()
    (interactive)
...
(defun hive-mcp-cider-eval-silent (code &optional timeout)
...
(defun hive-mcp-cider--eval-with-heartbeat (code &optional timeout-override)
