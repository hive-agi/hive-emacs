...
(defun hive-mcp-swarm-terminal-detect-buffer-backend (buffer)
  ...
  (when (buffer-live-p buffer)
    ...
    (cond
  ((and (derived-mode-p 'vterm-mode) (bound-and-true-p claude-code-ide-session-id)) 'claude-code-ide)
...
(defun hive-mcp-swarm-terminal---find-slave-by-buffer (buffer)
  ...
  (when (and (buffer-live-p buffer) (boundp 'hive-mcp-swarm--slaves) (hash-table-p hive-mcp-swarm--slaves))
    ...
    (throw 'found id))) hive-mcp-swarm--slaves) nil)))
...
(defun hive-mcp-swarm-terminal---on-window-buffer-change (frame)
  ...
  (when hive-mcp-swarm-terminal-lazy-render
    ...
    (accept-process-output proc 0.1))))) nil frame)))
...
(defun hive-mcp-swarm-terminal---detect-error (buffer)
  ...
  (when (buffer-live-p buffer)
    ...
    (let* ((error-preview (when (string-match (concat "^.*" (regexp-quote pattern) ".*$") recent-text)
    (substring recent-text (match-beginning 0) (min (match-end 0) (+ (match-beginning 0) 200))))))
    (throw 'found-error (cons error-type (or error-preview pattern))))))) nil))))))
...
(defun hive-mcp-swarm-terminal---truncate-for-preview (text max-length)
  ...
  (if (and text (> (length text) max-length)) (concat (substring text 0 (- max-length 3)) "...") (or text "")))

(defun hive-mcp-swarm-terminal---send-vterm (buffer text)
  "Send TEXT to vterm BUFFER, then return. Non-blocking."
  (with-current-buffer buffer
    (vterm-send-string text)
    (run-at-time hive-mcp-swarm-terminal-send-delay nil (lambda ()
    (condition-case err
    (when (buffer-live-p buffer)
    (with-current-buffer buffer
    (vterm-send-return)))
...
(defun hive-mcp-swarm-terminal-wait-ready (buffer callback &optional timeout-secs)
  ...
  (let* ((timeout (or timeout-secs hive-mcp-swarm-terminal-ready-timeout))
        ...
        (check-interval 0.2)
        (timer nil))
    (setq timer (run-with-timer 0 check-interval (lambda ()
    (condition-case err
    (cond
  ((not (buffer-live-p buffer)) (progn
  (cancel-timer timer)
  (funcall callback buffer nil)))
  ((hive-mcp-swarm-terminal-ready-p buffer) (progn
  (cancel-timer timer)
  (funcall callback buffer t)))
  ((> (- (float-time) start-time) timeout) (progn
  (cancel-timer timer)
  (message "[swarm-terminal] Timeout waiting for ready: %s" (buffer-name buffer))
  (funcall callback buffer nil))))
