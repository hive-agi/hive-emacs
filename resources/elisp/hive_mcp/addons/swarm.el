...
(defun hive-mcp-swarm---format-slave-detail (id slave)
  ...
  (clel-seq (clel-concat (list (clel-seq (clel-concat (list 'user/slave-id) (list '.) (list 'user/id)))) (list (clel-seq (clel-concat (list 'clojure.core/name) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :name))))))) (list (clel-seq (clel-concat (list 'user/status) (list '.) (list (clel-seq (clel-concat (list 'user/-get-slave-status-name) (list 'user/slave))))))) (list (clel-seq (clel-concat (list 'user/depth) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :depth))))))) (list (clel-seq (clel-concat (list 'user/parent-id) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :parent-id))))))) (list (clel-seq (clel-concat (list 'user/cwd) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :cwd))))))) (list (clel-seq (clel-concat (list 'user/current-task) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :current-task))))))) (list (clel-seq (clel-concat (list 'user/tasks-completed) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :tasks-completed))))))))))
...
(defun hive-mcp-swarm---format-ling-data (id slave)
  ...
  (clel-seq (clel-concat (list (clel-seq (clel-concat (list 'user/slave-id) (list '.) (list 'user/id)))) (list (clel-seq (clel-concat (list 'clojure.core/name) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :name))))))) (list (clel-seq (clel-concat (list 'user/presets) (list '.) (list (clel-seq (clel-concat (list 'user/-get-presets-or-empty) (list 'user/slave))))))) (list (clel-seq (clel-concat (list 'user/cwd) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/slave) (list :cwd))))))) (list (clel-seq (clel-concat (list 'user/status) (list '.) (list (clel-seq (clel-concat (list 'user/-get-slave-status-name) (list 'user/slave))))))))))
...
(defun hive-mcp-swarm---format-orphan-detail (orphan)
  ...
  (clel-seq (clel-concat (list (clel-seq (clel-concat (list 'user/slave-id) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/orphan) (list :slave-id))))))) (list (clel-seq (clel-concat (list 'clojure.core/name) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/orphan) (list :name))))))) (list (clel-seq (clel-concat (list 'user/status) (list '.) (list "orphan")))) (list (clel-seq (clel-concat (list 'user/depth) (list '.) (list 'nil)))) (list (clel-seq (clel-concat (list 'user/parent-id) (list '.) (list 'nil)))) (list (clel-seq (clel-concat (list 'user/current-task) (list '.) (list 'nil)))) (list (clel-seq (clel-concat (list 'user/tasks-completed) (list '.) (list 'nil)))) (list (clel-seq (clel-concat (list 'user/orphan) (list '.) (list 'user/t)))) (list (clel-seq (clel-concat (list 'user/discovered-via) (list '.) (list "buffer-introspection")))))))

(defun hive-mcp-swarm---format-orphan-ling (orphan)
  "Format ORPHAN plist as alist for lings list API response."
  (clel-seq (clel-concat (list (clel-seq (clel-concat (list 'user/slave-id) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/orphan) (list :slave-id))))))) (list (clel-seq (clel-concat (list 'clojure.core/name) (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/orphan) (list :name))))))) (list (clel-seq (clel-concat (list 'user/presets) (list '.) (list (apply clojure-core-vector (clel-seq (clel-concat))))))) (list (clel-seq (clel-concat (list 'user/cwd) (list '.) (list 'nil)))) (list (clel-seq (clel-concat (list 'user/status) (list '.) (list "orphan")))) (list (clel-seq (clel-concat (list 'user/orphan) (list '.) (list 'user/t)))))))
...
(defun hive-mcp-swarm---format-pending-prompt (prompt)
  ...
  (clel-seq (clel-concat (list :slave-id) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/prompt) (list :slave-id)))) (list :prompt) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/prompt) (list :prompt)))) (list :timestamp) (list (clel-seq (clel-concat (list 'user/format-time-string) (list "%Y-%m-%dT%H:%M:%S") (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/prompt) (list :timestamp))))))))))
...
(defun hive-mcp-swarm---build-status-response (total idle working error-count orphan-count slaves-detail)
  ...
  (clel-seq (clel-concat (list :session-id) (list 'user/hive-mcp-swarm--session-id) (list :status) (list (clel-seq (clel-concat (list 'if) (list (clel-seq (clel-concat (list 'clojure.core/>) (list 'user/total) (list 0)))) (list "active") (list "inactive")))) (list :current-depth) (list 'user/hive-mcp-swarm--current-depth) (list :safeguards) (list (clel-seq (clel-concat (list :max-depth) (list 'user/hive-mcp-swarm-max-depth) (list :max-slaves) (list 'user/hive-mcp-swarm-max-slaves) (list :rate-limit) (list (clel-seq (clel-concat (list :window-seconds) (list 'user/hive-mcp-swarm-rate-limit-window) (list :max-spawns) (list 'user/hive-mcp-swarm-rate-limit-max-spawns) (list :recent-spawns) (list (clel-seq (clel-concat (list 'user/length) (list 'user/hive-mcp-swarm--spawn-timestamps)))))))))) (list :slaves) (list (clel-seq (clel-concat (list :total) (list 'user/total) (list :idle) (list 'user/idle) (list :working) (list 'user/working) (list :error) (list 'user/error-count) (list :orphan) (list 'user/orphan-count)))) (list :tasks) (list (clel-seq (clel-concat (list :total) (list (clel-seq (clel-concat (list 'user/hash-table-count) (list 'user/hive-mcp-swarm--tasks))))))) (list :slaves-detail) (list 'user/slaves-detail))))
...
(defun hive-mcp-swarm-api-spawn (name presets &optional cwd terminal kanban-task-id context-file)
  ...
  (let* ((valid-cwd (when (stringp cwd)
    ...
    (condition-case err-5022
    (hive-mcp-swarm-spawn name :presets presets :cwd valid-cwd :terminal (when (and terminal (stringp terminal))
    (intern terminal)) :kanban-task-id valid-kanban-id :injected-context injected-context)
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5022)
      (clel-seq (clel-concat (list :error) (list "spawn-failed") (list :name) (list 'clojure.core/name) (list :reason) (list "unknown")))))))
...
(defun hive-mcp-swarm-api-dispatch (slave-id prompt &optional timeout-ms)
  ...
  (condition-case err-5023
    (hive-mcp-swarm-dispatch slave-id prompt :timeout timeout-ms)
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5023)
      (clel-seq (clel-concat (list :status) (list "error") (list :error) (list "dispatch-failed") (list :slave-id) (list 'user/slave-id) (list :reason) (list "unknown"))))))
...
(defun hive-mcp-swarm-api-collect (task-id &optional timeout-ms)
  ...
  (condition-case err-5024
    ...
    (cond
  (((not task) (clel-seq (clel-concat (list :task-id) (list 'user/task-id) (list :status) (list "error") (list :error) (list "Task not found")))) ((eq (plist-get task :status) 'completed) (clel-seq (clel-concat (list :task-id) (list 'user/task-id) (list :status) (list "completed") (list :result) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/task) (list :result))))))))
  (((eq (plist-get task :status) 'timeout) (clel-seq (clel-concat (list :task-id) (list 'user/task-id) (list :status) (list "timeout") (list :error) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/task) (list :error))))))) ((and elapsed-ms (> elapsed-ms task-timeout)) (plist-put task :status 'timeout) (plist-put task :error (format "Timed out after %dms" elapsed-ms)) (when slave
    (plist-put slave :status 'idle)
    (plist-put slave :current-task nil)
    (plist-put slave :tasks-failed (1+ (or (plist-get slave :tasks-failed) 0)))) (hive-mcp-swarm--emit-task-failed task-id slave-id (format "Timed out after %dms" elapsed-ms)) (clel-seq (clel-concat (list :task-id) (list 'user/task-id) (list :status) (list "timeout") (list :error) (list (clel-seq (clel-concat (list 'clojure.core/format) (list "Timed out after %dms") (list 'user/elapsed-ms)))) (list :elapsed-ms) (list 'user/elapsed-ms)))))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5024)
      (clel-seq (clel-concat (list :task-id) (list 'user/task-id) (list :status) (list "error") (list :result) (list 'nil) (list :error) (list "collection-failed"))))))
...
(defun hive-mcp-swarm-api-kill (slave-id)
  ...
  (condition-case err-5025
    (if (hive-mcp-swarm-kill slave-id) (clel-seq (clel-concat (list :killed) (list 'user/slave-id) (list :success) (list 'user/t))) (clel-seq (clel-concat (list :error) (list "kill-blocked") (list :slave-id) (list 'user/slave-id) (list :reason) (list "buffer-safety-validation-failed") (list :message) (list "Buffer did not pass safety checks (may have been coordinator or invalid target)"))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5025)
      (clel-seq (clel-concat (list :error) (list "kill-failed") (list :slave-id) (list 'user/slave-id))))))

(defun hive-mcp-swarm-api-kill-all ()
  "API: Kill all slaves.\nReturns result plist. Never fails."
  (condition-case err-5026
    (let* ((count (hash-table-count hive-mcp-swarm--slaves)))
    (hive-mcp-swarm-kill-all)
    (clel-seq (clel-concat (list :killed-count) (list 'clojure.core/count))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5026)
      (clel-seq (clel-concat (list :error) (list "kill-all-failed") (list :killed-count) (list 0))))))
...
(defun hive-mcp-swarm-api-pending-prompts ()
  ...
  (condition-case err-5027
    (let* ((pending (hive-mcp-swarm-prompts-get-pending))
        (prompts (mapcar #'-format-pending-prompt pending)))
    (clel-seq (clel-concat (list :count) (list (clel-seq (clel-concat (list 'user/length) (list 'user/prompts)))) (list :prompts) (list 'user/prompts) (list :mode) (list 'user/hive-mcp-swarm-prompts-mode))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5027)
      (clel-seq (clel-concat (list :error) (list "pending-prompts-failed") (list :count) (list 0) (list :prompts) (list 'nil))))))
...
(defun hive-mcp-swarm-api-respond-prompt (slave-id response)
  ...
  (condition-case err-5028
    (if (hive-mcp-swarm-prompts-respond-to slave-id response) (clel-seq (clel-concat (list :success) (list 'user/t) (list :slave-id) (list 'user/slave-id) (list :response) (list 'user/response))) (clel-seq (clel-concat (list :success) (list 'nil) (list :error) (list "no-pending-prompt") (list :slave-id) (list 'user/slave-id))))
  (error (hive-mcp-graceful--log-error "fallback triggered" err-5028)
      (clel-seq (clel-concat (list :error) (list "respond-prompt-failed") (list :slave-id) (list 'user/slave-id))))))
