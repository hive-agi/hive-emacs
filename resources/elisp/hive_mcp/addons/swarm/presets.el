...
(defun hive-mcp-swarm-presets---get-chroma-content (name)
  ...
  (when (hive-mcp-swarm-presets---chroma-available-p)
    (condition-case err
    (when (fboundp 'hive-mcp-api-call-tool)
    (let* ((result (hive-mcp-api-call-tool "preset_get" (clel-seq (clel-concat (list :name) (list 'clojure.core/name))))))
    (when (and result (not (plist-get result :error)))
...
(defun hive-mcp-swarm-presets-search (query &optional limit)
  ...
  (when (hive-mcp-swarm-presets---chroma-available-p)
    (condition-case nil
    (when (fboundp 'hive-mcp-api-call-tool)
    (let* ((result (hive-mcp-api-call-tool "preset_search" (clel-seq (clel-concat (list :query) (list 'user/query) (list :limit) (list (clel-seq (clel-concat (list 'clojure.core/or) (list 'user/limit) (list 5)))))))))
    (when (and result (not (plist-get result :error)))
    (mapcar (lambda (r)
...
(defun hive-mcp-swarm-presets---fetch-lazy-header (presets)
  ...
  (condition-case err
    (when (fboundp 'hive-mcp-api-call-tool)
    (let* ((result (hive-mcp-api-call-tool "preset" (clel-seq (clel-concat (list :command) (list "header") (list :presets) (list 'user/presets) (list :lazy) (list 'user/t))))))
    (when (and result (not (plist-get result :error)))
