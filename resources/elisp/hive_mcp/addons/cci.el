...
(defun hive-mcp-cci---handle-hivemind-completion (task-id status result files)
  ...
  (when-let-star (list task (gethash task-id hive-mcp-cci--tasks)) (plist-put task :status (intern (or status "success"))) (plist-put task :result result) (plist-put task :files-modified files) (plist-put task :completed-at (format-time-string "%FT%T%z")) (when-let-star (list ling-id (plist-get task :ling-id) ling (gethash ling-id hive-mcp-cci--lings)) (plist-put ling :status 'idle) (plist-put ling :current-task nil) (cl-incf (plist-get ling :tasks-completed))) (when-let-star (list callback (gethash task-id hive-mcp-cci--pending-completions)) (remhash task-id hive-mcp-cci--pending-completions) (funcall callback task)) (hive-mcp-cci---emit-event "task-completed" (clel-seq (clel-concat (list (clel-seq (clel-concat (list "task-id") (list '.) (list 'user/task-id)))) (list (clel-seq (clel-concat (list "status") (list '.) (list 'user/status)))) (list (clel-seq (clel-concat (list "ling-id") (list '.) (list (clel-seq (clel-concat (list 'user/plist-get) (list 'user/task) (list :ling-id)))))))))) (message "[cci] Task %s completed via hivemind: %s" task-id status)))
...
(defun hive-mcp-cci---emit-event (event-type data)
  ...
  (when (and (fboundp 'hive-mcp-channel-connected-p) (hive-mcp-channel-connected-p))
    (ignore-errors (hive-mcp-channel-send (clel-seq (clel-concat (list (clel-seq (clel-concat (list "type") (list '.) (list (clel-seq (clel-concat (list 'clojure.core/format) (list "cci-%s") (list 'user/event-type))))))) (list (clel-seq (clel-concat (list "data") (list '.) (list 'user/data)))) (list (clel-seq (clel-concat (list "timestamp") (list '.) (list (clel-seq (clel-concat (list 'user/format-time-string) (list "%FT%T%z")))))))))))))
...
(defun hive-mcp-cci-status ()
  ...
  ((quote error) (cl-incf error-count)))
    ...
    (let* ((status (clel-seq (clel-concat (list :backend) (list "claude-code-ide") (list :completion-mechanism) (list "hivemind") (list :auto-sync) (list 'user/hive-mcp-cci-auto-sync) (list :lings) (list (clel-seq (clel-concat (list :total) (list 'user/total) (list :idle) (list 'user/idle) (list :working) (list 'user/working) (list :error) (list 'user/error-count)))) (list :tasks) (list (clel-seq (clel-concat (list :total) (list (clel-seq (clel-concat (list 'user/hash-table-count) (list 'user/hive-mcp-cci--tasks)))) (list :pending) (list (clel-seq (clel-concat (list 'user/hash-table-count) (list 'user/hive-mcp-cci--pending-completions))))))) (list :lings-detail) (list (clel-seq (clel-concat (list 'user/nreverse) (list 'user/lings-detail))))))))
...
(defun hive-mcp-cci-api-spawn (name presets &optional cwd)
  ...
  (error (clel-seq (clel-concat (list :error) (list "spawn-failed") (list :reason) (list (clel-seq (clel-concat (list 'user/error-message-string) (list 'user/err)))))))))
...
(defun hive-mcp-cci-api-dispatch (ling-id prompt &optional timeout-ms)
  ...
  (error (clel-seq (clel-concat (list :error) (list "dispatch-failed") (list :reason) (list (clel-seq (clel-concat (list 'user/error-message-string) (list 'user/err)))))))))
...
(defun hive-mcp-cci-api-collect (task-id)
  ...
  (if-let-star (list task (gethash task-id hive-mcp-cci--tasks)) task (clel-seq (clel-concat (list :error) (list "task-not-found") (list :task-id) (list 'user/task-id)))))
