;;; hive-mcp-melpazoid.el --- -*- lexical-binding: t; -*-
...
(defcustom hive-mcp-melpazoid-path nil
  ...
  :type '(choice (const :tag "Auto-detect" nil) (directory :tag "Path")))
...
(defvar hive-mcp-melpazoid--common-paths '("~/dotfiles/gitthings/melpazoid" "~/melpazoid" "~/projects/melpazoid" "~/.local/share/melpazoid")
  "Common paths to search for melpazoid.")

(defun hive-mcp-melpazoid---find-path ()
  "Find melpazoid installation path."
  (or hive-mcp-melpazoid-path (seq-find (lambda (p)
    (let* ((expanded (expand-file-name p)))
    (and (file-directory-p expanded) (file-exists-p (expand-file-name "Makefile" expanded))))) hive-mcp-melpazoid--common-paths) (error "Melpazoid not found. Set `hive-mcp-melpazoid-path' or clone from https://github.com/riscy/melpazoid")))
...
(defun hive-mcp-melpazoid---parse-output (output)
  ...
  (let* ((results (list :errors '() :warnings '() :info '() :sections '())))
    (let* ((pos 0))
...
(defun hive-mcp-melpazoid---format-summary (results)
  ...
  (format "Melpazoid Results:\n  Errors:   %d\n  Warnings: %d\n  Status:   %s\n  Sections: %s" (plist-get results :error-count) (plist-get results :warning-count) (if (plist-get results :success) "PASS" "FAIL") (mapconcat #'identity (plist-get results :sections) ", ")))
...
(defun hive-mcp-melpazoid-run (project-dir &optional recipe)
  ...
  ('_ "FULL Docker rebuild"))))
    ...
    (insert "=" (make-string 60 -p-eq) "\n\n")
    (insert (if (eq hive-mcp-melpazoid-fast-mode 'local) "Running melpazoid locally...\n\n" "Running melpazoid (this may take a few minutes)...\n\n"))))
    (display-buffer buf)
    (when (and hive-mcp-melpazoid--process (process-live-p hive-mcp-melpazoid--process))
    (kill-process hive-mcp-melpazoid--process))
    (setq hive-mcp-melpazoid--process (start-process-shell-command "melpazoid" buf cmd))
    (set-process-sentinel hive-mcp-melpazoid--process (lambda (proc event)
...
(defun hive-mcp-melpazoid-show-results ()
  ...
  (interactive)
  (if-let-star (list buf (get-buffer hive-mcp-melpazoid--buffer-name)) (display-buffer buf) (message "No melpazoid results available. Run M-x hive-mcp-melpazoid-run first")))

(defun hive-mcp-melpazoid-save-results ()
  "Save last melpazoid results to MCP memory."
  (interactive)
  (if hive-mcp-melpazoid--last-results (let* ((content (with-current-buffer (get-buffer hive-mcp-melpazoid--buffer-name)
    (buffer-string))))
    (hive-mcp-api-memory-add "note" content '("melpazoid" "melpa"))
    (message "Saved melpazoid results to memory")) (message "No melpazoid results to save")))

(defun hive-mcp-melpazoid-status ()
  "Return the current melpazoid status."
  (interactive)
  (let* ((status (list :running (and hive-mcp-melpazoid--process (process-live-p hive-mcp-melpazoid--process)) :has-results (and hive-mcp-melpazoid--last-results t) :last-success (when hive-mcp-melpazoid--last-results
    (plist-get hive-mcp-melpazoid--last-results :success)) :last-errors (when hive-mcp-melpazoid--last-results
    (plist-get hive-mcp-melpazoid--last-results :error-count)) :last-warnings (when hive-mcp-melpazoid--last-results
    (plist-get hive-mcp-melpazoid--last-results :warning-count)))))
    (when (called-interactively-p 'any)
    (message "Melpazoid: %s, Last run: %s (%d errors, %d warnings)" (if (plist-get status :running) "running" "idle") (if (plist-get status :has-results) (if (plist-get status :last-success) "PASS" "FAIL") "no results") (or (plist-get status :last-errors) 0) (or (plist-get status :last-warnings) 0)))
    status))

(defun hive-mcp-melpazoid-cycle-fast-mode ()
  "Cycle through fast mode options: nil -> local -> cached -> nil."
  (interactive)
  (setq hive-mcp-melpazoid-fast-mode (pcase hive-mcp-melpazoid-fast-mode
  ((quote nil) 'local)
  ((quote local) 'cached)
  ((quote cached) nil)))
