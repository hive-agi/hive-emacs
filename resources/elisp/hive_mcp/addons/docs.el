...
(defun hive-mcp-docs-apropos (pattern &optional type)
  ...
  ('_ t))) all-symbols))
    ...
    (list :name (symbol-name sym) :type (hive-mcp-docs---symbol-type sym) :docstring-preview (when-let-star (list doc (hive-mcp-docs---get-docstring sym)) (if (> (length doc) 100) (concat (substring doc 0 100) "...") doc)))) limited))))
...
(defun hive-mcp-docs-find-keybindings (command)
  ...
  (let* ((sym (if (symbolp command) command (intern-soft command))))
    ...
    (list :command (symbol-name sym) :docstring-preview (when-let-star (list doc (documentation sym t)) (if (> (length doc) 200) (concat (substring doc 0 200) "...") doc)) :bindings (mapcar (lambda (key)
...
(defun hive-mcp-docs-package-commentary (file-or-feature)
  ...
  (t nil)))
        (file (when lib-file
    (if (string-suffix-p ".elc" lib-file) (let* ((src (concat (file-name-sans-extension lib-file) ".el")))
...
(defun hive-mcp-docs-list-packages ()
  ...
  (let* ((features-info (mapcar (lambda (feat)
    (list :name (symbol-name feat) :file (when-let-star (list file (locate-library (symbol-name feat))) (file-name-nondirectory file)))) features)))
...
(defun hive-mcp-docs-apropos-interactive (pattern)
  ...
  (let* ((result (hive-mcp-docs-apropos pattern)))
    ...
    (when-let-star (list doc (plist-get sym :docstring-preview)) (insert (format "    %s\n" doc))))
    (goto-char (point-min)))
...
(defun hive-mcp-docs-package-functions-interactive (prefix)
  ...
  (let* ((result (hive-mcp-docs-package-functions prefix)))
    ...
    (insert (format "%s%s %s\n" (if (plist-get fn :interactive) "*" " ") (plist-get fn :name) (or (plist-get fn :signature) ""))))
    (insert "\n* = interactive command\n")
    (goto-char (point-min)))
