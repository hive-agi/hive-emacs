...
(defun hive-mcp-ellama--tool-read (path)
  ...
  (when (fboundp 'hive-mcp-api-call)
    (hive-mcp-api-call "read_file" (clel-seq (clel-concat (list :path) (list 'user/path))))))

(defun hive-mcp-ellama--tool-edit (path old-string new-string)
  "Edit file at PATH, replacing OLD-STRING with NEW-STRING via MCP."
  (when (fboundp 'hive-mcp-api-call)
    (hive-mcp-api-call "file_edit" (clel-seq (clel-concat (list :file_path) (list 'user/path) (list :old_string) (list 'user/old-string) (list :new_string) (list 'user/new-string))))))

(defun hive-mcp-ellama--tool-bash (command)
  "Run shell COMMAND via MCP."
  (when (fboundp 'hive-mcp-api-call)
    (hive-mcp-api-call "bash" (clel-seq (clel-concat (list :command) (list 'user/command))))))
