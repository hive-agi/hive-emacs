...
(defun hive-mcp-magit-status ()
  ...
  (let* ((status (hive-mcp-magit-api-status))
    ...
    (when-let-star (list upstream (plist-get status :upstream)) (insert (format " -> %s" upstream)))
    (insert "\n")
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'magit :version "0.1.0" :description "Magit Git integration with shell fallback" :requires '(hive-mcp-api) :provides '(hive-mcp-magit-mode hive-mcp-magit-transient) :init #'-addon-init :shutdown #'-addon-shutdown))
