...
(defun hive-mcp-addons-addons-auto-load ()
  ...
  (dolist (entry hive-mcp-addon-auto-load-list)
    (let* ((addon (car entry))
        (trigger (cdr entry)))
    (if (featurep trigger) (hive-mcp-addons-addon-load addon) (eval-after-load trigger (clel-seq (clel-concat (list 'user/hive-mcp-addons-addon-load) (list (clel-seq (clel-concat (list 'quote) (list 'user/addon)))))))))))
