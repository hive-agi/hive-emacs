...
(defun hive-mcp-presentation-api-get-info (file)
  ...
  (with-temp-buffer
    ...
    (let* ((format (hive-mcp-presentation--detect-format)))
    (list (cons 'file file) (cons 'format format) (cons 'html-exists (file-exists-p (concat (file-name-sans-extension file) ".html"))) (cons 'pdf-exists (file-exists-p (concat (file-name-sans-extension file) ".pdf")))))))
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register 'presentation :version "0.1.0" :description "Org-mode presentation creation (Beamer & Reveal.js)" :requires '(hive-mcp-api org) :provides '(hive-mcp-presentation-transient) :init #'hive-mcp-presentation--addon-init :shutdown #'hive-mcp-presentation--addon-shutdown))
